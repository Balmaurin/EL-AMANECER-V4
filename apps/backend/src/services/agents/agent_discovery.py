...
class AgentDiscoveryService:
    ...
    def _scan_available_imports(self) -> None:
        ...
        # Importar AgentOrchestrator y sus agentes
        try:
            from apps.backend.src.core.agent_orchestrator import agent_orchestrator
            
            # Obtener agentes del orchestrator
            orchestrator_agents = agent_orchestrator.agents
            logger.info(f"🔍 Importando agentes del AgentOrchestrator: {len(orchestrator_agents)}")
            
            for agent_id, agent_def in orchestrator_agents.items():
                agent_info = {
                    "id": agent_id,
                    "name": agent_def.name,
                    "type": agent_def.agent_type.value if hasattr(agent_def.agent_type, 'value') else str(agent_def.agent_type),
                    "module": "apps.backend.src.core.agent_orchestrator",
                    "file_path": "apps/backend/src/core/agent_orchestrator.py",
                    "description": agent_def.description,
                    "capabilities": [cap.value if hasattr(cap, 'value') else str(cap) for cap in agent_def.capabilities],
                    "methods": [],
                    "status": "active",
                    "last_seen": self.last_scan,
                }
                self.discovered_agents.append(agent_info)
                logger.info(f"✅ Agente registrado: {agent_def.name}")
                
        except Exception as e:
            logger.warning(f"No se pudo importar AgentOrchestrator: {e}")
        
        # Módulos conocidos adicionales
        known_agent_modules = [
...
# Auto-scan on import to populate agents immediately
try:
    import datetime
    agent_discovery.last_scan = datetime.datetime.now().isoformat()
    agent_discovery.scan_all_agents()
    logger.info(f"🎯 Agent Discovery initialized: {len(agent_discovery.discovered_agents)} agents found")
except Exception as e:
    logger.warning(f"Could not auto-initialize agent discovery: {e}")
