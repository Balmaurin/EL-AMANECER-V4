...
# Load sheily_core via MCP Module Loader
try:
    from apps.backend.src.core.mcp_module_loader import mcp_module_loader
    
    sheily_core = mcp_module_loader.get_module("sheily_core")
    
    if sheily_core:
        # Try importing from loaded module
        ...
        # Sheily core not available, use stubs
        ...
        print("⚠️ Sheily Core not available - dashboard using mock implementation")
    ...
    SHEILY_CORE_AVAILABLE = False
    print(f"⚠️ MCP Module Loader not available for dashboard: {e}")
