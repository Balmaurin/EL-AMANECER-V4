...
# Import ToM system via MCP Module Loader
try:
    from apps.backend.src.core.mcp_module_loader import mcp_module_loader
    
    # Get ToM from MCP loader
    unified_tom = mcp_module_loader.get_module("theory_of_mind")
    TOM_AVAILABLE = unified_tom is not None
    
    if TOM_AVAILABLE:
        print("✅ Theory of Mind loaded via MCP Module Loader")
    else:
        print("⚠️  Theory of Mind not available (MCP will manage it)")
        print("   Note: ToM endpoints will return 503 for advanced features")
    
except ImportError as e:
    print(f"⚠️  MCP Module Loader not available: {e}")
    unified_tom = None
    TOM_AVAILABLE = False
