...
from apps.backend.src.core.security.rate_limiter import RateLimiter
...
# Rate limiter orquestación (in-memory mode without redis)
orchestration_limiter = RateLimiter(enable_redis=False)
...
async def get_mcp_status():
    ...
    """
    try:
        # Obtener agentes DIRECTAMENTE del AgentOrchestrator
        from apps.backend.src.core.agent_orchestrator import agent_orchestrator
        
        # Contar agentes del orchestrator
        total_agents = len(agent_orchestrator.agents)
        active_agents = len([a for a in agent_orchestrator.agent_instances.values() if a])
        active_orchestrations = len(orchestration_manager.active_orchestrations)
        
        # Calcular health score basado en agentes disponibles
        health_score = 0.2  # Base
        if total_agents > 0:
            health_score += 0.3  # +30% si hay agentes
        if active_agents > 0:
            health_score += 0.3  # +30% si hay agentes activos
        if active_orchestrations == 0:
            health_score += 0.2  # +20% si no hay errores
        
        return MCPStatusResponse(
            master_agent_active=total_agents > 0,
            total_agents=total_agents,
            active_agents=max(active_agents, total_agents),  # Al menos contar los definidos
            system_health_score=min(health_score, 1.0),
