...
class SpecializedBranchesLoader:
    ...
    def __init__(
        ...
        branches_root: str,
        corpus_root: str,
        cache_db: str = "specialized_branches.db",
    ):
        ...
        logger.info(f"🌳 SpecializedBranchesLoader inicializado")
        logger.info(f"   • Ramas: {self.branches_root}")
        logger.info(f"   • Corpus: {self.corpus_root}")
        logger.info(f"   • Cache: {self.cache_db}")
    ...
    def scan_all_branches(self) -> Dict[str, Dict]:
        ...
        logger.info("🔍 Escaneando ramas especializadas...")
        branches_found = {}
        ...
        logger.info(f"✅ Encontradas {len(branches_found)} ramas especializadas")
    ...
    def _analyze_branch(self, branch_path: Path) -> Optional[Dict]:
        ...
        try:
            ...
            branch_info["specialization_keywords"] = self._extract_domain_keywords(
                branch_path.name
            )
            ...
            logger.error(f"⚠️  Error analizando rama {branch_path.name}: {e}")
            return None
    ...
    def _extract_domain_keywords(self, branch_name: str) -> List[str]:
        ...
        for domain, domain_kw in domain_keywords.items():
            if domain in normalized or any(
                kw.replace(" ", "_") in normalized for kw in domain_kw
            ):
    ...
    def load_corpus_documents(self) -> Dict[str, List[Dict]]:
        ...
        logger.info("📚 Cargando documentos del corpus...")
        ...
        logger.info(
            f"✅ Cargados {total_docs} documentos en {len(corpus_docs)} dominios"
        )
    ...
    def _load_domain_documents(self, domain_path: Path) -> List[Dict]:
        ...
        try:
            ...
            logger.error(f"⚠️  Error cargando documentos de {domain_path.name}: {e}")
        return documents

    def build_domain_router(self) -> Dict[str, Dict]:
        """Construir router de dominios inteligente"""
        logger.info("🧠 Construyendo router de dominios...")
        router = {}
        ...
        self.conn.commit()
        logger.info(f"✅ Router construido para {len(router)} dominios")
        return router
    ...
    def _calculate_priority(self, branch_info: Dict) -> Dict:
        ...
        priority = {
            ...
            "keyword_bonus": min(
                0.2, len(branch_info["specialization_keywords"]) / 20.0
            ),
    ...
    def _calculate_domain_score(self, query: str, router_info: Dict) -> float:
        ...
        partial_matches = sum(
            1
            for kw in rules.get("partial_match", [])
            if any(part in query for part in kw.split())
        )
        score += partial_matches * 0.2

        # Boost semántico (keywords importantes)
        semantic_matches = sum(
            1 for kw in rules.get("semantic_boost", []) if kw in query
        )
    ...
    def index_all_documents(self):
        ...
        logger.info("🔄 Indexando documentos especializados...")
        branches = self.scan_all_branches()
        ...
        logger.info(f"✅ Indexados {total_indexed} documentos especializados")
    ...
    def _index_branch_dataset(self, branch_name: str, branch_info: Dict) -> int:
        ...
        try:
                ...
                for i, line in enumerate(f):
                    try:
                        ...
                        if content.strip():
                            ...
                            spec_score = self._calculate_specialization_score(
                                content, keywords
                            )
            ...
            logger.error(f"⚠️  Error indexando rama {branch_name}: {e}")
    ...
    def get_stats(self) -> Dict:
        ...
        cursor = self.conn.execute(
            "SELECT COUNT(DISTINCT domain) FROM domain_documents"
        )
        total_domains = cursor.fetchone()[0]

        cursor = self.conn.execute(
            "SELECT COUNT(DISTINCT branch_name) FROM domain_documents"
        )
...
def main():
    ...
    loader = SpecializedBranchesLoader(
        branches_root="branches", corpus_root="corpus_ES"
    )

    logger.info("🔄 Iniciando carga completa del sistema...")
    ...
    logger.info(f"   • Documentos totales: {stats['total_documents']}")
    logger.info(f"   • Dominios: {stats['total_domains']}")
    logger.info(f"   • Ramas: {stats['total_branches']}")
    logger.info("\n🏆 TOP DOMINIOS:")
    ...
    for query in test_queries:
        ...
        logger.info(f"   • '{query}' → {domain} (score: {score:.3f})")
