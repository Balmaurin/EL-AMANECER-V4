...
def save_adapter_config_to_file(
    config: AdapterConfig, config_path: Path
) -> Result[Path, str]:
...
def compose_adapter_configs(
    ...
    merged_modules = list(
        set(base_config.target_modules + overlay_config.target_modules)
    )
...
def calculate_adapter_memory_size(config: AdapterConfig) -> int:
    ...
    total_memory = len(config.target_modules) * (
        base_memory_per_module + rank_multiplier
    )
    return total_memory
...
def filter_adapters_by_language(
    ...
    return [
        adapter for adapter in adapters if adapter.metadata.get("language") == language
    ]
...
def sort_adapters_by_performance(
    ...
    return sorted(
        adapters, key=lambda a: get_avg_performance(a.adapter_id), reverse=True
    )
...
def create_adapter_processor(
    adapters_path: Path,
) -> Callable[[str], Optional[AdapterConfig]]:
...
def create_adapter_filter(
    language: str,
) -> Callable[[List[AdapterConfig]], List[AdapterConfig]]:
...
def create_adapter_pipeline(
    ...
    def pipeline(
        base_adapter_id: str, overlay_adapter_id: str
