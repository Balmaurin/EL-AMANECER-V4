...
class BranchManager:
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            logger.info(
                f"BranchManager inicializado con {len(self.branches)} ramas disponibles"
            )
            return True
    ...
    async def query_branch(self, query: BranchQuery) -> BranchResponse:
        ...
        try:
            ...
            await self._update_branch_stats(
                query.branch_name, response, processing_time
            )
                ...
                if branch_path.is_dir() and not branch_path.name.startswith("."):
                    ...
                    if config_file.exists():
                        try:
                            ...
                            discovered_branches.append(
                                {
                                    ...
                                    "domain": branch_config.get(
                                        "domain", branch_path.name
                                    ),
                            ...
                            logger.warning(
                                f"Error cargando config de rama {branch_path.name}: {e}"
                            )
    ...
    async def _create_branch_info(self, branch_data: Dict) -> BranchInfo:
        ...
        if "corpus_path" in branch_data and Path(branch_data["corpus_path"]).exists():
            corpus_size = await self._calculate_corpus_size(
                Path(branch_data["corpus_path"])
            )
    ...
    async def _execute_branch_query(self, query: BranchQuery) -> BranchResponse:
        ...
        if branch_info.type == BranchType.TECHNICAL:
            ...
            response_text = (
                f"[Análisis académico en {branch_info.domain}] {query.query}"
            )
            ...
            response_text = (
                f"[Análisis especializado en {branch_info.domain}] {query.query}"
            )
        else:
    ...
    def _calculate_query_confidence(
        self, query: BranchQuery, branch_info: BranchInfo
    ) -> float:
    ...
    async def _apply_specialization(
        ...
        try:
            ...
            response.confidence = max(
                response.confidence, specialized_response.confidence
            )
    ...
    async def _update_branch_metrics(self):
        ...
        for branch_info in self.branches.values():
            ...
            if branch_info.usage_count > 10:
                ...
                branch_info.specialization_level = (
                    usage_factor + accuracy_factor
                ) / 2.0
