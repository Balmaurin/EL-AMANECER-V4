...
class BranchSelector:
    ...
    def __init__(self, config: Dict):
        ...
        logger.info(
            f"BranchSelector inicializado con {len(self.available_branches)} ramas"
        )
    ...
    async def select_branch(
        ...
        try:
            ...
            if self.strategy == SelectionStrategy.KEYWORD_BASED:
                ...
                result = await self._semantic_similarity_selection(
                ...
                result = await self._ml_classification_selection(
                    query, language, domain
                )
            else:
        ...
        for branch_name in self.available_branches:
            ...
            for keyword in branch_keywords:
                if keyword.lower() in query_lower:
                    ...
                    keyword_weight = (
                        len(keyword) / 10.0
                    )  # Keywords más largos = más específicos
    ...
    async def _semantic_similarity_selection(
        ...
        if best_branch[1] > 0.2:  # Threshold mínimo
            ...
            return SelectionResult(
                ...
                metadata={
                    "similarity_score": best_branch[1],
                    "method": "jaccard_similarity_mock",
                },
            )
        else:
            return SelectionResult(
    ...
    async def _hybrid_selection(
        ...
        alternatives = sorted(
            all_alternatives.values(), key=lambda x: x.confidence, reverse=True
        )[: self.max_alternatives]
        ...
        return SelectionResult(
            ...
            metadata={
                ...
                "consensus": keyword_result.selected_branch
                == semantic_result.selected_branch,
    ...
    def _extract_query_features(self, query: str, language: str) -> Dict[str, Any]:
        ...
        features = {
            ...
            "question_words": len(
                re.findall(
                    r"\b(qué|cómo|cuándo|dónde|por qué|what|how|when|where|why)\b",
                    query.lower(),
                )
            ),
            "technical_terms": len(
    ...
    def _real_ml_classification(self, features: Dict[str, Any]) -> str:
        ...
        if features["technical_terms"] > 0:
            ...
            branch_scores["inteligencia artificial"] += (
                features["technical_terms"] * 2.0
            )
    ...
    def _calculate_classification_confidence(
        ...
        if features["word_count"] > 30 and predicted_branch in [
            "filosofía",
            "literatura",
        ]:
            confidence += 0.2
