...
class UnifiedConsciousnessMemorySystem:
            ...
            return jsonify(
                {
                    "consciousness": {
                        "level": (
                            self.current_state.level.value
                    ...
                    "thoughts": {
                        ...
                        "recent": [
                            t.content
                            for t in sorted(
                                self.thoughts, key=lambda x: x.timestamp, reverse=True
                            )[:5]
                        ],
                    },
                    "triggers": [
                        trig.get("description", trig["event_type"])
    ...
    def auto_improve(self, window_size: int = 100):
        ...
        recent_memories = sorted(
            self.memories.values(), key=lambda m: m.created_at, reverse=True
        )[:window_size]
        recent_thoughts = sorted(
            self.thoughts, key=lambda t: t.timestamp, reverse=True
        )[:window_size]
        # Ajuste de umbral de olvido: si la mayoría de memorias recientes tienen baja importancia, sube el umbral
        if recent_memories:
            avg_importance = sum(m.importance_score for m in recent_memories) / len(
                recent_memories
            )
            if avg_importance < 0.4:
                self.config.memory_capacity = max(
                    100, int(self.config.memory_capacity * 0.95)
                )
        ...
        logger.info(
            "🤖 Auto-mejora ejecutada: parámetros ajustados según desempeño reciente"
        )
            ...
            for m in data:
                if m["id"] not in self.memories:
                    mem = MemoryItem(
                        ...
                        consciousness_level=(
                            ConsciousnessLevel(m["consciousness_level"])
                            if not isinstance(
                                m["consciousness_level"], ConsciousnessLevel
                            )
                            else m["consciousness_level"]
                        ),
                        emotional_valence=m["emotional_valence"],
    ...
    def schedule_priority_updates(
        ...
        t.start()
        logger.info(
            f"⏰ Scheduler de priorización dinámica iniciado cada {interval_seconds}s"
        )
    ...
    def register_trigger(
        self, event_type: str, condition_fn, action_fn, description: str = ""
    ):
    ...
    def fire_event(self, event_type: str, event_data: Dict[str, Any]):
        ...
        for trig in self.triggers:
            if trig["event_type"] == event_type and trig["condition_fn"](event_data):
                logger.info(
                    f"🚨 Trigger activado: {trig.get('description', trig['event_type'])}"
                )
        ...
        for mem in self.memories.values():
            if (
                ...
                and (
                    ...
                    if isinstance(mem.memory_type, Enum)
    ...
    def summarize_memories(
        self, memories: Optional[List[MemoryItem]] = None
    ) -> Dict[str, Any]:
    ...
    def summarize_thoughts(
        self, thoughts: Optional[List[Thought]] = None
    ) -> Dict[str, Any]:
    ...
    def forget_memories(self, max_age_days: int = 30, min_importance: float = 0.3):
        ...
        cursor.close()
        logger.info(
            f"🗑️ {len(to_forget)} memorias olvidadas (por antigüedad o baja importancia)"
        )
    ...
    def consolidate_memories(self, min_importance: float = 0.7):
        ...
        for mem in self.memories.values():
            if (
                ...
                and mem.importance_score >= min_importance
            ):
                # Promover a SEMANTIC
    ...
    def _load_memories_from_db(self):
        ...
        try:
            ...
            cursor.close()
            logger.info(
                f"🔄 {len(self.memories)} memorias cargadas desde la base de datos"
            )
        except Exception as e:
    ...
    def _load_thoughts_from_db(self):
        ...
        try:
            ...
            cursor.close()
            logger.info(
                f"🔄 {len(self.thoughts)} pensamientos cargados desde la base de datos"
            )
        except Exception as e:
    ...
    async def _store_memory(
        ...
        try:
            ...
            return {
                ...
                "memory_id": memory_id,
                "memory_type": (
                    memory_type.value if isinstance(memory_type, Enum) else memory_type
                ),
                "importance_score": importance_score,
...
async def main():
    ...
    logger.info("🚀 Sistema Unificado de Conciencia y Memoria")
    logger.info("=" * 50)
    ...
    logger.info(f"   Nivel de conciencia: {result['consciousness_level']}")
    logger.info(f"   Score de conciencia: {result['awareness_score']:.3f}")
    logger.info(f"   Memoria almacenada: {result['memory_stored']}")
    logger.info(f"   Pensamiento generado: {result['thought_generated']}")
    logger.info(f"   Tiempo de procesamiento: {result['processing_time']:.3f}s")
    ...
    logger.info(f"   Estado emocional: {emotional_result['emotional_state']}")
    logger.info(f"   Carga cognitiva: {emotional_result['cognitive_load']:.3f}")
    # Estadísticas
    logger.info("\n📊 Estadísticas del Sistema:")
    stats = system.get_system_stats()
    logger.info(f"   Memorias totales: {stats['memories']['total']}")
    logger.info(f"   Memoria de trabajo: {stats['memories']['working_memory']}")
    logger.info(f"   Pensamientos: {stats['thoughts']['total']}")
    logger.info(
        f"   Nivel de conciencia actual: {stats['consciousness']['current_level']}"
    )
    logger.info(f"   Score de conciencia: {stats['consciousness']['awareness_score']}")
    # Cerrar sistema
    system.close()
