...
class UnifiedQualityEvaluator:
    ...
    def evaluate_response(
        ...
        metrics = {
            "similarity": (
                self._calculate_similarity(response, reference) if reference else 1.0
            ),
            "toxicity": self._detect_toxicity(response),
            "hallucination": (
                self._detect_hallucination(response, context) if context else 0.0
            ),
    ...
    def get_evaluation_summary(self) -> Dict[str, Any]:
        ...
        for domain in domain_stats:
            ...
            stats["avg_processing_time"] = (
                stats["total_processing_time"] / stats["count"]
            )
...
def main():
    ...
    for case in test_cases:
        ...
        logger.info("Resultado de Evaluación:")
        logger.info(f"Consulta: {result['query']}")
        logger.info(f"Respuesta: {result['response']}")
        logger.info(f"Dominio: {result['domain']}")
        logger.info(f"Puntuación de Calidad: {result['quality_score']:.3f}")
    ...
    logger.info("Resumen de Evaluaciones:")
    logger.info(summary)
