...
class UnifiedModulesManager:
    ...
    def _load_modules_config(self):
        ...
        if not MODULES_CONFIG:
            logger.warning(
                "⚠️ No se pudo cargar MODULES_CONFIG, usando configuración vacía"
            )
            return
    ...
    async def _load_module(self, name: str, config: ModuleConfig) -> Optional[Any]:
        ...
        try:
            ...
            if config.special_init == "circuit_breaker":
                from sheily_core.utils.functional_errors import CircuitBreakerStrategy

                cb_config = CircuitBreakerStrategy(
                    failure_threshold=5, recovery_timeout=60
                )
                instance = cls("neurofusion", cb_config)

            elif config.special_init == "retry_handler":
                from sheily_core.utils.functional_errors import RetryStrategy

                policy = RetryStrategy(max_attempts=3, base_delay=1.0)
                instance = cls(policy)
...
async def main():
    ...
    logger.info("🎯 Unified Modules Manager - Demostración")
    logger.info("=" * 70)
    # Crear configuración
    ...
    logger.info(f"\n📊 Estadísticas:")
    logger.info(f"   Módulos totales: {stats['metrics']['total_modules']}")
    logger.info(f"   Módulos activos: {stats['metrics']['active_modules']}")
    logger.info(f"   Funciones totales: {stats['metrics']['total_functions']}")
    # Mostrar módulos por categoría
    categories = manager.get_modules_by_category()
    logger.info(f"\n📦 Módulos por categoría:")
    for category, modules in categories.items():
        if modules:
            logger.info(f"   {category}: {len(modules)} módulos")
    # Apagar
