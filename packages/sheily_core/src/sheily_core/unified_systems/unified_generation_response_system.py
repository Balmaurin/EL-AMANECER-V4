...
class GenerationResult:
    ...
    id: str
    content: str
    prompt: str  # Prompt real usado para la generación
...
class UnifiedGenerationResponseSystem:
    ...
    def __init__(
        self, config: Optional[GenerationConfig] = None, db_path: Optional[str] = None
    ):
    ...
    def _create_tables(self):
        ...
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_quality_score ON generations(quality_score)"
        )
        cursor.execute(
    ...
    async def generate_response(self, request: GenerationRequest) -> GenerationResult:
        ...
        try:
            ...
            quality_score = self._calculate_quality_score(
                final_content, validation_result
            )
            ...
            result = GenerationResult(
                ...
                prompt=adapted_prompt,  # Almacenar el prompt real usado (adaptado o original)
    ...
    async def _save_generation_to_db(self, result: GenerationResult):
        ...
        try:
            ...
            """,
                (
                    result.id,
                    result.prompt,  # Almacenar el prompt real usado
                    result.content,
            ...
            return {
                ...
                "performance": {
                    "generation_types": list(
                        set(
                            gen.generation_type.value for gen in self.generation_history
                        )
                    ),
                    "response_modes": list(
...
async def main():
    ...
    logger.info("🚀 Sistema Unificado de Generación y Respuesta")
    logger.info("=" * 50)
    ...
    logger.info(f"   Contenido: {text_result.content}")
    logger.info(f"   Calidad: {text_result.quality_score:.3f}")
    logger.info(f"   Confianza: {text_result.confidence:.3f}")
    logger.info(f"   Tiempo: {text_result.processing_time:.3f}s")
    ...
    logger.info(f"   Resultado: {classification_result.content}")
    if classification_result.validation_results:
        logger.info(
            f"   Válido: {classification_result.validation_results.get('is_valid', 'N/A')}"
        )
    else:
        logger.info(f"   Válido: N/A")
    ...
    logger.info(f"   Contenido: {creative_result.content}")
    logger.info(f"   Refinamientos: {len(creative_result.refinements)}")
    # Estadísticas
    logger.info("\n📊 Estadísticas del Sistema:")
    stats = system.get_system_stats()
    logger.info(f"   Generaciones totales: {stats['generations']['total']}")
    logger.info(f"   Calidad promedio: {stats['generations']['average_quality']}")
    logger.info(f"   Tasa de éxito validación: {stats['validations']['success_rate']}")
    logger.info(f"   Refinamientos totales: {stats['refinements']['total']}")
    # Cerrar sistema
    system.close()
