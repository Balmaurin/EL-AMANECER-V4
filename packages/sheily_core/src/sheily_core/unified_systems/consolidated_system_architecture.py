...
# Conditional import for sqlalchemy
try:
    ...
    Boolean = Column = DateTime = Float = Integer = String = Text = create_engine = None
...
if HAS_SQLALCHEMY and Base:
    ...
    User = QualityEvaluation = EmbeddingRecord = SystemMetrics = None
...
class UnifiedEmbeddingSystem:
    ...
    def __init__(self, config: UnifiedSystemConfig):
        ...
        try:
            if HAS_SENTENCE_TRANSFORMERS:
                ...
                logger.info(
                    f"✅ Modelo de embeddings cargado: {config.embedding_model_name}"
                )
            else:
                logger.warning(
                    "⚠️ sentence_transformers no disponible, embeddings deshabilitados"
                )
    ...
    async def _save_embedding_record(
        self, content: str, embedding: np.ndarray, domain: str
    ):
...
class UnifiedMonitoringSystem:
    ...
    async def _monitoring_loop(self):
        ...
        while True:
            try:
                ...
                await self.record_metric(
                    "memory_usage", len(self.metrics_buffer), "monitoring"
                )
    ...
    async def _check_alerts(
        self, metric_name: str, metric_value: float, component: str
    ):
...
class NeuroFusionUnifiedSystem:
    ...
    async def process_query(
        ...
        try:
            ...
            query_embedding = await self.embedding_system.generate_embedding(
                query, domain
            )
...
async def main():
    ...
    logger.info("🚀 Iniciando Sistema NeuroFusion Unificado")
    logger.info("=" * 50)
    # Crear configuración
    ...
    for i, test_case in enumerate(test_queries, 1):
        ...
        try:
            ...
            logger.info(f"   📊 Calidad: {result['evaluation']['overall_score']:.3f}")
            logger.info(f"   ⏱️  Tiempo: {result['processing_time']:.3f}s")
            if result["evaluation"]["issues"]:
                logger.warning(
                    f"   ⚠️  Problemas: {', '.join(result['evaluation']['issues'])}"
                )
        except Exception as e:
    ...
    logger.info(f"   Estado: {status['health']['status']}")
    logger.info(f"   Métricas totales: {status['health']['total_metrics']}")
    logger.info(f"   Componentes: {len(status['health']['component_stats'])}")
