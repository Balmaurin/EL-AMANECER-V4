...
class ContextualAccuracyEvaluator:
    ...
    def linguistic_coverage(self, query: str, response: str) -> float:
        ...
        query_key_words = set(
            token.lemma_ for token in doc_query if token.pos_ in ["NOUN", "VERB"]
        )
    ...
    def contextual_precision(self, query: str, response: str) -> float:
        ...
        contextual_score = (
            0.6 * semantic_sim
            + 0.4
            * linguistic_cov  # Similitud semántica más importante  # Cobertura lingüística
        )
...
if __name__ == "__main__":
    ...
    logger.info("Precisión Contextual - Respuesta Correcta:")
    logger.info(evaluator.contextual_precision(query, response1))
    logger.info("\nPrecisión Contextual - Respuesta Incorrecta:")
    logger.info(evaluator.contextual_precision(query, response2))
