...
class AdvancedFileProcessor:
    ...
    def read_docx(file_path: Path) -> str:
        ...
        if not DOCX_AVAILABLE:
            return (
                f"[DOCX - Instalar: pip install python-docx]\nArchivo: {file_path.name}"
            )
    ...
    def read_csv(file_path: Path) -> str:
        ...
        try:
            ...
            df = pd.read_csv(
                file_path, delimiter=delimiter, encoding="utf-8", on_bad_lines="skip"
            )
    ...
    def read_video(file_path: Path) -> str:
        ...
        try:
            ...
            content.append(
                f"Duración: {duration:.2f} segundos ({duration/60:.2f} minutos)"
            )
    ...
    def read_json(file_path: Path) -> str:
        ...
        try:
            ...
            if isinstance(data, dict):
                content.append(
                    f"Claves principales: {', '.join(list(data.keys())[:10])}"
                )
...
class SheilyMemoryIntegrator:
    ...
    def __init__(self):
        ...
        self.input_dir = Path(
            "/home/yo/Escritorio/Sheily-Final/memoria_entrada/pendientes"
        )
        self.processed_dir = Path(
            "/home/yo/Escritorio/Sheily-Final/memoria_entrada/completados"
        )
    ...
    def process_memory_integration(self) -> Dict[str, Any]:
        ...
        logger.info("🧠 SISTEMA DE MEMORIA SHEILY - PROCESAMIENTO EXTENDIDO")
        logger.info("=" * 70)
        ...
        for file_info in input_files:
            logger.info(f"📄 Procesando: {file_info['file_name']}")
            logger.info(
                f"   Tamaño: {file_info['file_size_mb']:.2f} MB | Tipo: {file_info['extension']}"
            )
            # Validar archivo
            is_valid, validation_message = self.validate_file(file_info)

            if not is_valid:
                logger.error(f"   ❌ {validation_message}\n")
            ...
            try:
                ...
                logger.info(
                    f"   ✓ Chunks creados: {memory_entry['metadata']['total_chunks']}"
                )
                logger.info(f"   ✓ Categoría: {memory_entry['context']['category']}")
                logger.info(
                    f"   ✓ Tags: {', '.join(memory_entry['context']['tags'][:3])}"
                )
                # Guardar memoria
                if self.save_memory(memory_entry):
                    ...
                    logger.info(f"   ✅ Integrado exitosamente [ID: {memory_id}]\n")
                else:
                    file_info["status"] = "save_error"
                    results["failed_integrations"] += 1
                    logger.error(f"   ❌ Error guardando memoria\n")
            except Exception as e:
        ...
        logger.info(
            f"   • Procesados exitosamente: {results['successful_integrations']}"
        )
        logger.error(f"   • Errores: {results['failed_integrations']}")
        logger.info(
            f"   • Total memorias en sistema: {self.memory_index['total_memories']}"
        )
        logger.info(f"   • Categorías: {len(self.memory_index['categories'])}")
        logger.info(f"   • Tags únicos: {len(self.memory_index['tags'])}")
        logger.info(f"   • Tipos de archivo: {len(self.memory_index['file_types'])}")
        return results
...
def print_dependencies_status():
    ...
    if not all([PDF_AVAILABLE, DOCX_AVAILABLE]):
        ...
        logger.info("pip install PyPDF2 python-docx")
    ...
    if not all([PIL_AVAILABLE, TESSERACT_AVAILABLE]):
        ...
        logger.info("pip install pillow pytesseract")
        logger.info("# Linux: sudo apt install tesseract-ocr tesseract-ocr-spa")
        logger.info("# Mac: brew install tesseract tesseract-lang")
        logger.info(
            "# Windows: Descargar de https://github.com/UB-Mannheim/tesseract/wiki"
        )
    if not SPEECH_AVAILABLE:
        logger.info("\n# Audio:")
        logger.info("pip install SpeechRecognition pydub")
    if not CV2_AVAILABLE:
        logger.info("\n# Video:")
        logger.info("pip install opencv-python")
    if not CHARDET_AVAILABLE:
        logger.info("\n# Utilidades:")
        logger.info("pip install chardet")
...
def main():
    ...
    if results["total_files"] == 0:
        ...
        logger.info("=" * 70)
        logger.info(f"1. Coloca archivos en: {integrator.input_dir}")
        logger.info("2. Ejecuta: python memory_integrator.py")
        logger.info("3. Los archivos procesados aparecerán en:")
        logger.info(f"   • Originales: {integrator.processed_dir}")
        logger.info(f"   • Memorias: {integrator.memory_dirs['processed']}")
        logger.info(f"   • Índice: {integrator.memory_index_file}")
        logger.info("\n📋 TIPOS DE ARCHIVO SOPORTADOS:")
        logger.info("   • Documentos: PDF, DOCX, TXT, MD, HTML, XML")
        logger.info("   • Datos: CSV, XLSX, XLS, JSON")
        logger.info("   • Código: PY, JS, JAVA, CPP, C, CSS, SQL, etc.")
        logger.info("   • Imágenes: PNG, JPG, JPEG, BMP, TIFF, GIF (con OCR)")
        logger.info("   • Audio: WAV, MP3, OGG, FLAC (con transcripción)")
        logger.info("   • Video: MP4, AVI, MKV, MOV, WMV (metadata)")
        logger.info("   • Archivos: ZIP, TAR, TAR.GZ")
        logger.info("=" * 70)
