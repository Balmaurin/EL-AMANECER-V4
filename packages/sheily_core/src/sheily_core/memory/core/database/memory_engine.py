...
class SheilyMemoryV2:
    ...
    def memorize_file(
        ...
        try:
            ...
            logger.error(f"Error memorizando archivo {file_path}: {e}")
            return False
    ...
    def _generate_simple_embedding(self, text: str) -> np.ndarray:
        ...
        features = [
            ...
            sum(ord(c) for c in text[:100])
            / 100,  # Valor promedio de caracteres iniciales
        ]
        ...
        try:
            ...
            for row in cursor.fetchall():
                similarity = self._calculate_similarity(
                    query_embedding, json.loads(row[3])
                )
    ...
    def _calculate_similarity(
        self, query_emb: np.ndarray, chunk_emb: np.ndarray
    ) -> float:
    ...
    def get_contextual_memories(
        ...
        try:
            ...
            logger.error(f"Error obteniendo recuerdos contextuales: {e}")
            return []
    ...
    def get_memory_health(self) -> Dict[str, Any]:
        ...
        health_score = min(
            100, stats["total_documents"] * 2 + stats["total_chunks"] * 0.5
        )
    ...
    def _get_health_recommendations(self, stats: Dict[str, Any]) -> List[str]:
        ...
        if stats["database_size_mb"] > 100:
            recommendations.append(
                "Considerar limpieza de documentos antiguos poco importantes"
            )

        if stats["average_importance"] < 0.4:
            recommendations.append(
                "Revisar criterios de importancia para documentos existentes"
            )
