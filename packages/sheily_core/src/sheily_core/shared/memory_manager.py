...
class MemoryManager:
    ...
    def initialize(self) -> bool:
        ...
        try:
            logger.info("🚀 Inicializando sistema de memoria unificado...")
            ...
            logger.info("✅ Sistema de memoria unificado operativo")
            return True
    ...
    def _init_human_memory(self):
        ...
        try:
            ...
            logger.info("✅ Memoria humana avanzada V2 operativa")
        except Exception as e:
            logger.error(f"⚠️ Error inicializando memoria humana: {e}")
            self.memory_engine = None
    ...
    def _init_rag_engine(self):
        ...
        try:
            ...
            logger.info("✅ Motor RAG neurológico operativo")
        except Exception as e:
    ...
    def search_memory(self, query: str, top_k: int = 5) -> List[Dict[str, Any]]:
        ...
        if self.memory_engine:
            try:
                ...
                logger.error(f"⚠️ Error en búsqueda de memoria: {e}")
        # Buscar en RAG
        if self.rag_engine:
            try:
    ...
    def memorize_content(
        ...
        if self.memory_engine:
            try:
                ...
                logger.error(f"⚠️ Error memorizando en memoria humana: {e}")
        # Indexar en RAG
        if self.rag_engine:
            try:
                ...
                logger.error(f"⚠️ Error indexando en RAG: {e}")
        return memory_ids
...
if __name__ == "__main__":
    # Test del módulo
    logger.info("🧪 Probando Memory Manager...")
    # Inicializar
    if _memory_manager.initialize():
        logger.info("✅ Memoria inicializada exitosamente")
        # Probar búsqueda
        results = search_memory("hola", top_k=3)
        logger.info(f"✅ Búsqueda realizada: {len(results)} resultados")
        # Probar memorización
        memory_ids = memorize_content("Test de memoria unificada", "test")
        logger.info(f"✅ Contenido memorizado: {len(memory_ids)} IDs")
        # Mostrar estadísticas
        stats = get_memory_stats()
        logger.info(f"✅ Estadísticas: {stats['initialized']}")
    else:
        logger.error("❌ Error inicializando memoria")
