...
class ServerManager:
    ...
    def check_server_running(self) -> bool:
        ...
        try:
            response = requests.get(
                f"http://{self.config.model.host}:{self.config.model.port}/health",
                timeout=2.0,
            )
    ...
    def start_server(self) -> bool:
        ...
        with self._lock:
            ...
            if self.check_server_running():
                logger.info("✅ Servidor ya activo, reutilizando...")
            ...
            if not server_path.exists():
                logger.error(f"❌ No se encontró llama-server en: {server_path}")
                return False
            ...
            logger.info("🚀 Iniciando servidor GGUF optimizado...")
            ...
            try:
                ...
                if self._wait_for_server():
                    ...
                    logger.error("❌ Timeout iniciando servidor")
    ...
    def _wait_for_server(self, timeout: int = 120) -> bool:
        ...
        while time.time() < deadline:
            ...
            if self.server_process.poll() is not None:
                logger.error("❌ Servidor terminó inesperadamente")
                return False
    ...
    def stop_server(self) -> bool:
        ...
        with self._lock:
            ...
            logger.info("🛑 Parando servidor GGUF...")
            self._cleanup_process()
            ...
            while self.server_status == "running":
                try:
                    if not self.check_server_running():
                        logger.warning(
                            "⚠️ Servidor no responde, intentando reiniciar..."
                        )
    ...
    def get_server_info(self) -> Dict[str, Any]:
        ...
        process_info = None
        if self.server_process and psutil is not None:
            try:
...
if __name__ == "__main__":
    # Test del módulo
    logger.info("🧪 Probando Server Manager...")
    # Verificar estado inicial
    logger.info(f"Estado inicial: {get_server_info()['status']}")
    # Iniciar servidor
    if start_server():
        ...
        if embedding:
            logger.info(f"✅ Embeddings funcionales: {len(embedding)} dimensiones")
        else:
