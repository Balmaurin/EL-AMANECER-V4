...
class HyperRouter:
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            await asyncio.gather(
                self.branch_selector.initialize(), self.load_balancer.initialize()
            )
    ...
    async def _execute_intelligent_routing(
        ...
        try:
            if analyzed_type == RouteType.RAG_SEARCH:
                ...
                result = await self._execute_llm_generation(
                    request, balanced_components
                )
    ...
    async def _analyze_query_type(self, query: str) -> RouteType:
        ...
        if any(
            word in query_lower for word in ["vs", "versus", "compare", "diferencia"]
        ):
            analytical_score += 2
    ...
    async def _select_components(
        self, route_type: RouteType, branch: Optional[str]
    ) -> List[str]:
        """
        ...
        while True:
            try:
                ...
                healthy_components = sum(
                    1 for status in component_health.values() if status
                )
    ...
    async def shutdown(self):
        ...
        try:
            ...
            await asyncio.gather(
                self.branch_selector.shutdown(), self.load_balancer.shutdown()
            )
