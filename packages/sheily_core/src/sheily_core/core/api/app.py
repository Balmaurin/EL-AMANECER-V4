...
class ChatRequest(BaseModel):
    ...
    context: Optional[Dict[str, Any]] = Field(
        default={}, description="Contexto de la conversación"
    )
    ...
    basic_metrics = {
        ...
        "requests": {
            ...
            "average_response_time": (
                system_metrics["total_processing_time"]
                / max(system_metrics["requests_count"], 1)
            ),
            "requests_per_minute": system_metrics["requests_count"]
            / max(uptime / 60, 1),
        },
        "sessions": {
            "currently_active": len(system_metrics["active_sessions"]),
            "active_session_ids": list(system_metrics["active_sessions"])[
                :10
            ],  # Limit for privacy
        },
    }
    ...
    if ULTRA_FAST_AVAILABLE:
        try:
            ...
            status_info["ultra_fast_performance"] = {
                ...
                "avg_response_time": perf_report["performance_metrics"][
                    "avg_response_time"
                ],
                "total_requests": perf_report["performance_metrics"]["total_requests"],
                "peak_memory_usage": perf_report["performance_metrics"][
                    "peak_memory_usage"
                ],
            }
        except Exception as e:
    ...
    try:
        ...
        return {
            ...
            "performance": {
                ...
                "index_size": (
                    len(file_searcher.index.file_paths)
...
async def sei_licore_ultra_fast_endpoint(
    query: str, context: Optional[Dict[str, Any]] = None
):
    """Endpoint de pensamiento ultra-rápido SEI-LiCore"""
    if not ULTRA_FAST_AVAILABLE:
        raise HTTPException(
            status_code=503, detail="SEI-LiCore ultra-fast not available"
        )
...
async def get_ultra_fast_performance():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Performance report error: {str(e)}"
        )
...
async def optimize_system_memory():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Memory optimization error: {str(e)}"
        )
...
async def get_search_index_status():
    ...
    try:
        ...
        last_updated = (
            file_searcher.index.last_updated if hasattr(file_searcher, "index") else 0
        )
        ...
        return {
            "index_status": {
                ...
                "index_current": (
                    file_searcher._is_index_current()
            ...
            "search_capabilities": {
                "fuzzy_search": (
                    file_searcher.enable_fuzzy_search
                    if hasattr(file_searcher, "enable_fuzzy_search")
                    else False
                ),
                "semantic_search": (
                    file_searcher.enable_semantic_search
        ...
        raise HTTPException(
            status_code=500, detail=f"Search index status error: {str(e)}"
        )
...
async def rebuild_search_index(force: bool = False):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Search index rebuild error: {str(e)}"
        )
