    ...
    try:
        ...
        try:
            ...
            if config.cors_origins == ["*"]:
                ...
                logger.info(
                    f"✅ CORS configured with {len(config.cors_origins)} specific origins"
                )
        except Exception as e:
...
def create_fallback_response(
    query: str, engine_type: str = "fallback", error: str = None
):
    ...
    if any(word in query_lower for word in ["hola", "hello", "hi", "buenos días"]):
        ...
        word in query_lower
        for word in ["python", "programación", "código", "desarrollo"]
    ):
        ...
        word in query_lower
        for word in ["ia", "inteligencia artificial", "machine learning", "ai"]
    ):
        ...
        word in query_lower for word in ["salud", "medicina", "síntomas", "enfermedad"]
    ):
            ...
            "metadata": {
                ...
                "query_analysis": {
                    ...
                    "language_detected": (
                        ...
                        if any(
                            word in query_lower
                            for word in ["qué", "cómo", "dónde", "cuándo"]
                        )
                        else "mixed"
                    ),
                },
...
def initialize_system(config_path: str = None) -> SystemContext:
    ...
    if rag_engine and rag_engine.load_documents():
        logger.info(
            f"RAG engine initialized with {len(rag_engine.documents)} documents"
        )
    elif rag_engine:
...
def create_signal_handler(
    context: SystemContext, shutdown_flag: List[bool]
) -> Callable:
...
def run_server_mode(context: SystemContext) -> int:
    ...
    try:
        ...
        context.logger.info(
            f"Starting server on {context.config.host}:{context.config.port}"
        )
...
def run_chat_interactive_mode(context: SystemContext, shutdown_flag: List[bool]) -> int:
    ...
    logger.info("🤖 Sheily AI Interactive Chat")
...
def run_test_mode(context: SystemContext) -> int:
    ...
    if context.rag_engine:
        ...
        try:
            ...
            logger.info(f"✅ RAG engine test passed ({len(results)} results)")
            tests_passed += 1
        except Exception as e:
    ...
    try:
        ...
        logger.info("✅ Chat engine test passed")
        tests_passed += 1
    except Exception as e:
        logger.error(f"❌ Chat engine test failed: {e}")
    ...
    try:
        ...
        logger.info("✅ Branch detection test passed")
        tests_passed += 1
    except Exception as e:
        logger.error(f"❌ Branch detection test failed: {e}")
...
def run_config_mode(context: SystemContext, action: str = "show") -> int:
    ...
    if action == "show":
        logger.info("🔧 Sheily Configuration")
        logger.info("=" * 50)
        # Show basic configuration
        logger.info(f"\n📁 SYSTEM:")
        logger.info(f"  System Name: {context.config.system_name}")
        logger.info(f"  Version: {context.config.version}")
        logger.info(f"  Environment: {context.config.environment}")
        logger.info(f"\n📁 SERVER:")
        logger.info(f"  Host: {context.config.host}")
        logger.info(f"  Port: {context.config.port}")
        logger.info(f"  API Prefix: {context.config.api_prefix}")
        logger.info(f"\n📁 MODEL:")
        logger.info(f"  Model Path: {context.config.model_path or 'Not configured'}")
        logger.info(f"  Max Tokens: {context.config.model_max_tokens}")
        logger.info(f"  Temperature: {context.config.model_temperature}")
        logger.info(f"\n📁 FEATURES:")
        for feature, enabled in context.config.features.items():
            logger.error(f"  {feature}: {'✅' if enabled else '❌'}")
        return 0
...
Examples:
  ...
  python main.py --config custom.json server  # Use custom config
    ...
    parser.add_argument(
        "mode", choices=["server", "chat", "test", "config"], help="Operation mode"
    )
    ...
    parser.add_argument(
        "--version",
        action="version",
        version="Sheily AI System 2.0.0 - Functional Edition",
    )
