...
class EnterpriseConfigManager:
    ...
    def __init__(self, config_file: str = None):
        ...
        Args:
            config_file: Ruta opcional al archivo de configuración YAML.
        """
        self.config_file = config_file or "config/enterprise_config.yaml"
        self.config: EnterpriseConfig | None = None
    ...
    def _load_config_file(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Intentar cargar como JSON
                ...
                self.logger.warning(
                    f"Formato de configuración no soportado: {config_path.suffix}"
                )
                return {}
    ...
    def _merge_configurations(self, file_config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        base_config = {
            ...
            "require_authentication": getattr(
                self, "_env_require_authentication", False
            ),
            ...
            "database_url": getattr(
                self, "_env_database_url", "sqlite:///data/sheily_ai.db"
            ),
    ...
    def _validate_configuration(self):
        ...
        if (
            ...
            and not Path(self.config.llama_binary_path).exists()
        ):
            self.logger.warning(
                f"Binario llama-cli no encontrado: {self.config.llama_binary_path}"
            )
        ...
        if self.config.max_queries_per_minute < 1:
            raise ValueError(
                f"Límite de consultas inválido: {self.config.max_queries_per_minute}"
            )
