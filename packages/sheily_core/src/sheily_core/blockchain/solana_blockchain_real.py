...
try:
    ...
    logging.info("✅ Solana SDK disponible - usando solders completo")
except ImportError:
    try:
    ...
    def transfer_tokens(
        ...
        try:
            ...
            if from_wallet.token_balance < amount:
                raise ValueError(
                    f"Balance insuficiente: {from_wallet.token_balance} < {amount}"
                )
            ...
            if SOLANA_AVAILABLE and self.connection_available:
                ...
                try:
                    ...
                    keypair = Keypair.from_secret_key(
                        base58.b58decode(from_wallet.private_key)
                    )
                    ...
                    if response.value:
                        ...
                        logger.info(
                            f"✅ Transacción confirmada: {transaction.signature}"
                        )
                    else:
                ...
                logger.info(
                    f"✅ Transacción simulada confirmada: {transaction.signature}"
                )
...
def test_solana_blockchain():
    ...
    logger.info(f"Wallet 1: {wallet1.public_key}")
    logger.info(f"Wallet 2: {wallet2.public_key}")
    # Mintear tokens
    ...
    logger.info(f"Transacción: {transaction.transaction_id}")
    logger.info(f"Estado: {transaction.status}")
    # Verificar balances
    balance1 = blockchain.get_user_balance(user1)
    balance2 = blockchain.get_user_balance(user2)

    logger.info(f"Balance User1: {balance1['token_balance']} tokens")
    logger.info(f"Balance User2: {balance2['token_balance']} tokens")
    # Estado de red
    network_status = blockchain.get_network_status()
    logger.info(f"Estado de red: {network_status}")
