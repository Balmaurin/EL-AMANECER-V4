...
class FaithfulnessMetric:
    ...
    def evaluate(self, test_case: EvaluationTestCase) -> MetricResult:
        ...
        if not test_case.context:
            ...
            explanation = (
                "Evaluación de consistencia interna (sin contexto proporcionado)"
            )
        else:
    ...
    def _evaluate_contextual_faithfulness(self, output: str, context: str) -> float:
        ...
        cosine_similarity = TextSimilarityAnalyzer.cosine_similarity_words(
            output, context
        )
    ...
    def _extract_themes(self, text: str) -> set:
        ...
        for theme, keywords in themes.items():
            ...
            if theme == "matemáticas":
                if (
                    has_numbers or has_math_ops or matches >= 1
                ):  # Más flexible para matemáticas
        ...
        for category, patterns in context_response_patterns.items():
            ...
            if context_matches >= 1:  # Al menos un indicador del contexto
                ...
                if response_matches > 0:
                    category_score = min(
                        1.0,
                        (response_matches / len(patterns["appropriate_responses"])) * 2,
                    )
...
class ContextualRelevancyMetric:
    ...
    def _evaluate_input_relevance(self, output: str, input_text: str) -> float:
        ...
        cosine_score = TextSimilarityAnalyzer.cosine_similarity_words(
            output, input_text
        )

        # Detectar si responde a la pregunta
        question_response_score = self._evaluate_question_response_match(
            output, input_text
        )
        ...
        elif direct_answer_bonus > 0.7 and (
            semantic_score > 0.4 or keyword_score > 0.4
        ):
            final_score = max(0.93, final_score)
        elif direct_answer_bonus > 0.5 and (
            semantic_score > 0.3 or keyword_score > 0.3
        ):
            final_score = max(0.9, final_score)
        elif direct_answer_bonus > 0.4 or (
            semantic_score > 0.4 and keyword_score > 0.2
        ):
            final_score = max(0.87, final_score)

        # NIVEL EXCELENTE: Clara intención de responder
        if (
            keyword_score > 0.4 or semantic_score > 0.3
        ) and question_response_score > 0.5:
            final_score = max(0.84, final_score)
    ...
    def _evaluate_context_relevance(self, output: str, context: str) -> float:
        ...
        word_similarity = TextSimilarityAnalyzer.cosine_similarity_words(
            output, context
        )
        ...
        if thematic_coherence > 0.6 or concept_similarity > 0.4:
            ...
            [concept_similarity > 0.1, word_similarity > 0.1, jaccard_similarity > 0.1]
        ):
    ...
    def _evaluate_question_response_match(self, output: str, input_text: str) -> float:
        ...
        appropriate_responses = {
            "what": lambda text: bool(
                re.search(r"\b(es|son|significa|define|llamado)\b", text)
            ),
            "how": lambda text: bool(
                re.search(r"\b(mediante|through|usando|steps|pasos)\b", text)
            ),
            "why": lambda text: bool(
                re.search(r"\b(porque|debido|reason|causa)\b", text)
            ),
            "when": lambda text: bool(
                re.search(r"\b(\d{4}|ayer|hoy|when|durante)\b", text)
            ),
            "where": lambda text: bool(
                re.search(r"\b(en|at|ubicado|lugar|place)\b", text)
            ),
            "who": lambda text: bool(
                re.search(r"\b(persona|people|autor|quien)\b", text)
            ),
        }
...
class FluencyMetric:
    ...
    def _evaluate_coherence(self, text: str) -> float:
        ...
        connector_count = sum(
            1 for s in sentences[1:] for conn in connectors if conn in s.lower()
        )
...
class CorrectnessMetric:
    ...
    def _evaluate_key_information_preservation(
    ...
    def _evaluate_conceptual_accuracy(self, actual: str, expected: str) -> float:
        ...
        actual_entities = {
            word for word in actual_lower.split() if word in important_entities
        }
        expected_entities = {
            word for word in expected_lower.split() if word in important_entities
        }
        ...
        concept_accuracy = (
            len(exact_matches) + semantic_matches * 0.7
        ) / total_expected
        ...
        for q_word, elements in question_types.items():
            if q_word in input_lower:
                element_score = sum(
                    ...
                    if any(kw in output_lower for kw in element.split())
                )
...
class DeepEvalEngine:
    ...
    def evaluate(self, model: str, data: List[Dict[str, str]]) -> Dict[str, Any]:
        ...
        """
        logger.info(f"🔬 Iniciando evaluación real de {len(data)} casos...")
        ...
        for metric_name, metric in self.metrics.items():
            logger.info(f"   Ejecutando {metric_name}...")
            metric_results = []
            ...
            results[metric_name] = {
                ...
                "pass_rate": (
                    passed_count / len(metric_results) if metric_results else 0.0
                ),
        ...
        logger.info(f"✅ Evaluación completada en {evaluation_time:.2f}s")
        logger.info(f"   Score general: {final_report['summary']['overall_score']:.3f}")
        logger.info(
