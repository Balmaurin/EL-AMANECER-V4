...
from sheily_core.data.document_processor import (
    DocumentProcessor,
    process_all_branches_corpus,
)
...
class RAGService:
    ...
    async def initialize(self):
        ...
        logger.info("✅ RAG Service inicializado con FAISS")
    ...
    def _setup_routes(self):
        ...
        async def process_corpus(request: ProcessCorpusRequest):
            ...
            try:
                # Procesar documentos
                chunks = process_all_branches_corpus(
                    request.corpus_path, request.domains
                )

                if not chunks:
                    raise HTTPException(
                        status_code=404,
                        detail="No se encontraron documentos para procesar",
                    )
                ...
                raise HTTPException(
                    status_code=500, detail=f"Error procesando corpus: {str(e)}"
                )
        ...
        async def add_document(request: AddDocumentRequest):
            ...
            try:
                ...
                chunks = self.document_processor.text_splitter.split_text(
                    request.content
                )

                if not chunks:
                    raise HTTPException(
                        status_code=400, detail="Contenido vacío o inválido"
                    )
        ...
        async def search(request: SearchRequest) -> List[SearchResult]:
            ...
            try:
                ...
                query_embedding = await self.embedding_manager.encode_text(
                    request.query
                )
                    ...
                    if idx < len(self.index_ids):  # Validar índice
                        search_results.append(
                            SearchResult(
                                ...
                                score=float(
                                    score
                                ),  # FAISS devuelve similitud coseno normalizada
                            )
                        )
        ...
        async def health():
            ...
            return {
                "status": "healthy",
                "embedding_model": (
                    self.embedding_manager.config.model_name
...
async def main():
    ...
    try:
        ...
        logger.info("✅ RAG Service listo para recibir conexiones")
        # Ejecutar servidor
        import uvicorn
        ...
        config = uvicorn.Config(
            ...
            host="0.0.0.0",
            port=8000,
            log_level="info",  # Unificado para Docker/Compose/Healthcheck
        )
        server = uvicorn.Server(config)

        logger.info("🌐 Iniciando servidor HTTP en http://0.0.0.0:8000")
        await server.serve()

    except Exception as e:
        logger.error(f"❌ Error fatal en RAG Service: {e}")
        import traceback
