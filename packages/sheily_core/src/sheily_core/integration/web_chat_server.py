...
# Configurar CORS para permitir acceso desde file:// (dashboard HTML)
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],  # Permitir todas las origines para desarrollo
...
def _startup_event() -> None:
    ...
    rag_base_url = os.environ.get("SHEILY_RAG_SERVICE_URL", "http://localhost:8000")
    rag_enabled = initialize_rag_service(rag_base_url)

    # Si no está disponible, intentar procesar corpus automáticamente
    if not rag_enabled:
        logger.warning(f"⚠️ RAG Service no disponible en {rag_base_url}")
        logger.info("💡 Asegúrate de que el RAG Service esté ejecutándose:")
        logger.info("   python start_rag_service.py")
        logger.info("   O establece SHEILY_RAG_SERVICE_URL si está en otro puerto")
...
async def chat_endpoint(req: ChatRequest) -> Dict[str, Any]:
    ...
    if req.use_rag and rag_enabled:
        try:
            context, relevant_chunks = get_rag_context(
                req.message, max_context_length=2000
            )
            ...
            logger.warning(f"Warning: RAG processing failed: {e}")
            context = ""
    ...
    if req.use_mcp and mcp_enabled:
        try:
            ...
            logger.warning(f"Warning: MCP processing failed: {e}")
    ...
    try:
        if LLM_PROVIDER == "llama_cpp":
            ...
            options = (
                ...
                if abs(req.temperature - 0.7) > 0.001
                else {}
            )
        ...
        for server_name in servers_to_use:
            if server_name in available_servers:
                try:
                    ...
                    enhanced_context += (
                        f"[{server_name}] Contexto adicional del servidor MCP.\n"
                    )
                except ImportError:
                    logger.warning(f"Warning: MCP server {server_name} not available")
                    continue
