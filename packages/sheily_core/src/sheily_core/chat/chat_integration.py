...
try:
    import requests
except (ImportError, ModuleNotFoundError):  # Manejo específico para imports faltantes
...
from .experimental.auto_recovery import (
    get_system_health,
...
class ChatSystemConfig:
    ...
    def __init__(self):
        ...
        default_server = (
            project_root / "tools" / "llama.cpp" / "build" / "bin" / "llama-server"
        )
        ...
        self.llama_server = Path(
            os.environ.get("LLAMA_SERVER_BIN", str(default_server))
        )
        ...
        try:
            self.threads = int(os.environ.get("SHEILY_THREADS", "")) or max(
                8, os.cpu_count() or 8
            )
        except Exception:
        ...
        self.batch_size = int(
            os.environ.get("SHEILY_BATCH", os.environ.get("SHEILY_BATCH_SIZE", "2048"))
        )
        self.ubatch_size = int(
            os.environ.get("SHEILY_UBATCH", os.environ.get("SHEILY_UBATCH_SIZE", "512"))
        )
        self.gpu_layers = int(
            os.environ.get(
                "SHEILY_GPU_LAYERS", os.environ.get("SHEILY_N_GPU_LAYERS", "0")
            )
        )  # por defecto CPU puro
        # KV-cache y memoria
        self.kv_unified = os.environ.get("SHEILY_KV_UNIFIED", "0") in (
            "1",
            "true",
            "True",
        )
        self.no_kv_offload = os.environ.get("SHEILY_NO_KV_OFFLOAD", "0") in (
            "1",
            "true",
            "True",
        )
        ...
        self.fast_mode = (
            os.environ.get("SHEILY_FAST_MODE", "0") == "1"
        )  # Por defecto DESACTIVADO para memoria completa
...
class IntegratedChatEngine:
    ...
    def initialize_system(self) -> Result[bool, SheilyError]:
        ...
        try:
            ...
            with self._init_lock:
                ...
                self.logger.info(
                    "🚀 Inicializando Chat Integrado con Sistema de Errores Funcionales"
                )
                ...
                if self.safe_memory_engine is None:
                    ...
                    if memory_result.is_err():
                        self.logger.warning(
                            f"Error inicializando memoria: {memory_result.error.message}"
                        )
                ...
                self.logger.info(
                    f"✅ Sistema inicializado: {'Listo' if self.system_ready else 'Con problemas'}"
                )
    ...
    def _check_llamacpp_health(self) -> bool:
        try:
            ...
            response = requests.get(
                f"http://{self.config.host}:{self.config.port}/health", timeout=3
            )
    ...
    def _check_ollama_health(self) -> bool:
        try:
            ...
            response = requests.get(
                f"http://{self.config.ollama_host}:{self.config.ollama_port}/api/version",
                timeout=3,
            )
            return response.status_code == 200
        except Exception:
    ...
    def _start_gguf_server(self) -> bool:
        ...
        try:
            ...
            if not self.config.model_path.exists():
                self.logger.error(
                    f"❌ No se encontró el modelo en: {self.config.model_path}"
                )
                return False
            ...
            while time.time() < deadline:
                ...
                if self._check_server_health():
                    self.logger.info(
                        f"✅ Servidor GGUF activo en http://{self.config.host}:{self.config.port}"
                    )
                    return True
                time.sleep(0.5)
    ...
    def _init_memory_system(self) -> Result[bool, SheilyError]:
        ...
        try:
            ...
            self.logger.info(
                "✅ Sistema de memoria humana V2 inicializado directamente"
            )
            return Ok(True)
    ...
    def process_message(self, message: str) -> Result[str, SheilyError]:
        ...
        try:
            ...
            if self.error_monitor:
                self.error_monitor.record_operation(
                    "chat_system", "process_message", True, 0.1
                )
    ...
    def _get_safe_memory_context(self, query: str) -> str:
        ...
        try:
            ...
            is_identity_question = any(
                keyword in query_lower for keyword in identity_keywords
            )
            ...
            if is_identity_question:
                ...
                memory_results = self.memory_engine.search_memory(
                    "INFORMACIÓN CRÍTICA: Mi nombre es Sheily", top_k=1
                )
                if memory_results:
    ...
    def _build_personality_context(self) -> str:
        ...
        try:
            if self.memory_engine:
                ...
                personality_memories = self.memory_engine.search_memory(
                    "personalidad", top_k=1
                )
...
def run_integrated_chat():
    ...
    logger.info("🤖 SHEILY NEURO CHAT V2 - COMPLETAMENTE INTEGRADO")
    logger.info("=" * 60)
    logger.error("✅ Sistema de manejo de errores funcionales activo")
    logger.info("✅ Recuperación automática habilitada")
    logger.info("✅ Monitoreo avanzado operativo")
    logger.info("✅ Integración completa con memoria y modelos")
    logger.info()
    ...
    try:
        ...
        logger.info(f"🛠️ Proveedor de inferencia: {prov}")
        if prov == "llamacpp":
            logger.info(f"   • Modelo GGUF: {engine.config.model_path}")
            logger.info(f"   • llama-server: {engine.config.llama_server}")
        elif prov == "ollama":
            logger.info(
                f"   • Ollama host: {engine.config.ollama_host}:{engine.config.ollama_port}"
            )
            logger.info(f"   • Modelo Ollama: {engine.config.ollama_model}")
    except Exception:
        pass
    ...
    try:
        ...
        if init_result.is_err():
            ...
            logger.error(f"❌ Error crítico inicializando sistema: {msg}")
            return 1

        if not engine.system_ready:
            logger.warning(
                "⚠️ Sistema inicializado con problemas - continuando con modo básico"
            )
        logger.info("🗨️ Chat listo. Escribe 'salir' para terminar.\n")
        while True:
            try:
                msg = input("Tú: ").strip()
                ...
                if msg.lower() in ["salir", "exit", "quit"]:
                    logger.info(
                        "👋 ¡Hasta luego! Gracias por usar Sheily Neuro Chat V2"
                    )
                    break
                ...
                if msg.lower() == "estado":
                    ...
                    logger.error(
                        f"   • Sistema listo: {'✅ Sí' if status['system_ready'] else '❌ No'}"
                    )
                    logger.error(
                        f"   • Servidor GGUF: {'✅ Activo' if status['server_running'] else '❌ Inactivo'}"
                    )
                    logger.error(
                        f"   • Sistema de errores: {'✅ Activo' if status['error_system_ready'] else '❌ Inactivo'}"
                    )
                    logger.info(
                        f"   • Salud del sistema: {status.get('system_health', 'unknown')}"
                    )
                    continue
                ...
                if msg.lower() == "salud":
                    try:
                        health_status = get_system_health()
                        logger.info(f"🏥 Salud del sistema: {health_status.value}")
                    except Exception as e:
                ...
                if msg.lower() == "metricas":
                    if engine.error_monitor:
                        ...
                        logger.error("📊 MÉTRICAS DE ERRORES:")
                        logger.info(
                            f"   • Operaciones totales: {metrics.get('total_operations', 0)}"
                        )
                        logger.error(
                            f"   • Tasa de error: {metrics.get('error_rate', 0):.2%}"
                        )
                        logger.info(
                            f"   • Tiempo promedio: {metrics.get('avg_response_time', 0):.3f}s"
                        )
                    else:
                        logger.error("❌ Sistema de métricas no disponible")
                    continue
...
def run_integrated_chat_once(message: Optional[str] = None) -> int:
    ...
    """
    logger.info("🤖 SHEILY NEURO CHAT V2 - MODO ÚNICO")
    logger.info("=" * 60)
    engine = IntegratedChatEngine()
    try:
        init_result = engine.initialize_system()
        if init_result.is_err():
            logger.error(
                f"❌ Error crítico inicializando sistema: {init_result.error.message}"
            )
            return 1
...
if __name__ == "__main__":
    ...
    logger.error(
        "✅ Chat integrado con sistema de errores funcionales cargado completamente"
    )
