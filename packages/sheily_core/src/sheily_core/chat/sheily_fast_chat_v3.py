...
class FastChatConfig:
    ...
    def __init__(self):
        ...
        self.model_path = Path(
            os.getenv(
                "SHEILY_GGUF",
                "/home/yo/Escritorio/Sheily-Final/models/gguf/llama-3.2.gguf",
            )
        )
...
class FastChatV3:
    ...
    def _start_server(self) -> bool:
        ...
        if not self.config.model_path.exists():
            logger.error(f"❌ No se encontró el modelo en: {self.config.model_path}")
            return False

        if self.config.verbose:
            logger.info("🚀 Iniciando servidor GGUF...")
        cmd = [
        ...
        while time.time() < deadline:
            ...
            if self._check_server():
                if self.config.verbose:
                    logger.info(
                        f"✅ Servidor activo en http://{self.config.host}:{self.config.port}"
                    )
                return True
            time.sleep(0.3)
    ...
    def initialize(self) -> bool:
        ...
        if self._check_server():
            ...
            if self.config.verbose:
                logger.info("✅ Servidor GGUF ya activo")
        else:
            # Intentar iniciar servidor
            self.server_ready = self._start_server()

        if not self.server_ready:
            logger.error("❌ No se pudo inicializar el servidor GGUF")
            return False

        init_time = time.time() - start_time
        if self.config.verbose:
            logger.info(f"⚡ Sistema listo en {init_time:.1f}s")
        return True
    ...
    def run(self):
        ...
        logger.info("🤖 SHEILY FAST CHAT V3")
        logger.info("=" * 50)
        logger.info("✨ Sistema optimizado para velocidad máxima")
        logger.info("💬 Chat natural sin complejidad innecesaria")
        logger.info()
        # Inicializar
        ...
        logger.info("💬 Chat listo. Escribe 'salir' para terminar.\n")
        ...
        while True:
            try:
                ...
                if user_input.lower() == "clear":
                    ...
                    logger.info("🗑️  Historial borrado")
                    continue
                ...
                if user_input.lower() == "historial":
                    ...
                    for i, (u, a) in enumerate(self.conversation_history, 1):
                        logger.info(f"  {i}. Tú: {u[:50]}...")
                        logger.info(f"     Sheily: {a[:50]}...")
                    logger.info()
                    continue
