...
"""
    ...
    def _load_branches_config(self) -> Dict:
        ...
        try:
            ...
            if branches_file.exists():
                ...
                logger.warning(
                    "⚠️ Archivo de ramas no encontrado, usando ramas por defecto"
                )
                return self._get_default_branches()
        except Exception as e:
            logger.error(f"⚠️ Error cargando ramas: {e}")
            return self._get_default_branches()
    ...
    def detect_specialized_branch(self, query: str) -> Tuple[str, float]:
        ...
        if branch_scores:
            ...
            self.logger.info(
                f"Rama detectada: '{best_branch}' (score: {best_score:.1f})"
            )
        ...
        if rag_pipeline:
            try:
                ...
                for doc in base_docs:
                    ...
                    branch_keywords = self.branches_config[branch_name].get(
                        "keywords", []
                    )
                ...
                self.logger.debug(
                    f"Encontrados {len(context_docs)} documentos especializados"
                )
    ...
    def generate_response_with_gguf(
        ...
        specialized_prompt = self._create_specialized_prompt(
            query, context_docs, branch_name
        )
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                if len(response) > 10:
                    self.logger.info(
                        f"Respuesta GGUF generada: {len(response)} caracteres"
                    )
                    return response
                else:
                ...
                self.logger.error(
                    f"Error GGUF (código {result.returncode}): {result.stderr}"
                )
                return self._generate_fallback_response(query, branch_name)
...
Respuesta especializada:"""
    ...
    def process_complete_query(self, query: str, rag_pipeline=None) -> Dict:
        ...
        with self.logger.context(**log_context.__dict__):
            ...
            context_docs = self.search_specialized_context(
                query, branch_name, rag_pipeline
            )

            # 3. Generar respuesta con GGUF Q4
            response = self.generate_response_with_gguf(
                query, context_docs, branch_name
            )
