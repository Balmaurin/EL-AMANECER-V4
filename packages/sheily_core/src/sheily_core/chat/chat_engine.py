...
def create_chat_context(config_path: str = None) -> ChatContext:
    ...
    # Functions are already defined in this file, no import needed
    ...
    # Opción 2: Cargar modelo local
        ...
        try:
            ...
            print(f"\n❌ ERROR CREATING MODEL INTERFACE: {e}")
            import traceback
            traceback.print_exc()
...
def validate_model_files(
    model_path: str, llama_binary_path: str
...
def create_prompt(query: str, context_docs: List[str] = None) -> str:
    ...
    # Ultra-simple prompt to avoid crashes - single line
    return f"Pregunta: {query} Respuesta:"
...
def execute_model_command(
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            # llama-cli output format:
            # [logs...]
            # [prompt echo]
            # [generated text]
            # llama_print_timings: ...
            
            # Strategy: Extract text between prompt and timing logs
            response = ""
            
            # Find where the prompt ends (look for the last occurrence)
            if prompt in full_output:
                # Split by prompt and take everything after the last occurrence
                parts = full_output.split(prompt)
                if len(parts) > 1:
                    text_after_prompt = parts[-1]
                    
                    # Remove timing logs (they start with "llama_print_timings:")
                    if "llama_print_timings:" in text_after_prompt:
                        response = text_after_prompt.split("llama_print_timings:")[0]
                    else:
                        response = text_after_prompt
                    
                    # Clean up
                    response = response.strip()
                    
                    # Remove common llama-cli artifacts
                    response = response.replace("Log end", "").strip()
                    response = response.replace("Log start", "").strip()
            
            # Fallback: if extraction failed, try to get any substantial text
            if len(response) < 10:
                # Look for lines that don't contain llama/main/build keywords
                lines = full_output.split('\n')
                clean_lines = []
                for line in lines:
                    line_lower = line.lower()
                    # Skip log lines
                    if any(keyword in line_lower for keyword in ['llama_', 'main:', 'log ', 'build =', 'seed =', 'loaded meta']):
                        continue
                    # Skip if it's the prompt itself
            ...
            return Err(
                f"Model execution failed (code {result.returncode}): {result.stderr}"
            )
...
def generate_model_response(
    ...
    print(f"\n📝 GENERATE_MODEL_RESPONSE CALLED")
...
def get_basic_knowledge(branch_name: str) -> List[str]:
    ...
    basic_knowledge = {
        "programación": [
            ...
            "Para debuggear código, usa logger statements, debugger integrado o herramientas como pdb.",
...
def create_error_response(
    query: str, error: str, processing_time: float
) -> ChatResponse:
...
def process_chat_query(
    query: str, client_id: str, context: ChatContext
) -> ChatResponse:
    """
    ...
    with context.logger.context(**log_context.__dict__):
        try:
            ...
            security_result = check_security_validation(
                query, client_id, context.security_monitor
            )

            if security_result.is_err():
                security_reason = security_result.unwrap_err()
                context.logger.warning(
                    f"Security violation from {client_id}: {security_reason}"
                )
                processing_time = time.time() - start_time
                return create_security_blocked_response(
                    query, security_reason, processing_time
                )
            ...
            # Step 3: Generate response - MUST have model interface
            if not context.model_interface:
                raise RuntimeError("Model interface not initialized. Llama is required.")
