...
from sheily_core.memory.sheily_memory_vault import vault
...
def handle_learning_input(content: str) -> str:
    ...
    base_dir = Path.cwd()  # Or specify a more restrictive base

    # Check if content is a file path
    try:
        # Try to sanitize as path
        potential_path = sanitize_path(
            content, base_dir, allowed_extensions=[".pdf", ".txt"], max_depth=15
        )
        path = potential_path
        ...
        if path.exists() and path.is_file():
            ...
            if ext == ".pdf":
                chunks = pdf.extract_chunks_with_meta(path, base_dir)
                total_ids = 0
                for ch in chunks:
                    txt = cleaner.clean_text(ch["text"])
                    ids = vault.remember_chunked(
                        txt, USER_ID, origin="docs", meta=ch["meta"]
                    )
                    total_ids += len(ids)
        ...
        # Not a valid/safe path, treat as text
        pass
...
def respond(message: str) -> str:
    ...
    if exact_q:
        ...
        return (
            f"He borrado {removed} fragmento(s) que contienen o se parecen a ese texto."
        )
