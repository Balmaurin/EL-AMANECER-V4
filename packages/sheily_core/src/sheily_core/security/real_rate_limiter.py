...
class RealRateLimiter:
    ...
    def check_rate_limit(self, client_id: str) -> Tuple[bool, Optional[str]]:
        ...
        with self._lock:
            ...
            if client.blocked_until > now:
                ...
                logger.warning(
                    f"Client {client_id} bloqueado por {remaining}s "
                    f"(bloqueos: {client.blocked_count})"
                )
            ...
            client.requests = [
                req_time for req_time in client.requests if req_time > window_start
            ]
            ...
            if len(client.requests) >= self.max_requests:
                ...
                logger.warning(
                    f"Client {client_id} excedió rate limit: "
                    f"{len(client.requests)}/{self.max_requests} requests"
                )

                return (
                    False,
                    f"Rate limit exceeded: {self.max_requests} requests/minute",
                )
    ...
    def get_client_stats(self, client_id: str) -> Dict:
        ...
        with self._lock:
            ...
            return {
                ...
                "blocked_until": (
                    client.blocked_until if client.blocked_until > now else None
                ),
    ...
    def get_global_stats(self) -> Dict:
        ...
        with self._lock:
            ...
            for client in self._clients.values():
                active_requests += sum(
                    1 for req in client.requests if req > window_start
                )
    ...
    def _cleanup_old_records(self, now: float):
        ...
        to_remove = [
            ...
            if (not record.requests or max(record.requests) < cleanup_threshold)
            and record.blocked_until < now
        ]
...
def get_rate_limiter(max_requests_per_minute: int = 60, **kwargs) -> RealRateLimiter:
    ...
    with _limiter_lock:
        if _rate_limiter is None:
            _rate_limiter = RealRateLimiter(
                max_requests_per_minute=max_requests_per_minute, **kwargs
            )
