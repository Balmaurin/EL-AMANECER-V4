...
def main():
    ...
    logger.error(f"Usuario creado: {'✅' if success else '❌'}")
    # Autenticar usuario
    success, message, session_token = auth.authenticate_user(
        "testuser", "SecurePass123!"
    )
    logger.error(f"Autenticación: {'✅' if success else '❌'} - {message}")
    if success:
        # Configurar MFA
        success, message, qr_uri = auth.setup_mfa("testuser")
        logger.error(f"MFA configurado: {'✅' if success else '❌'} - {message}")
        if success:
            logger.info(f"QR URI: {qr_uri}")
...
def main():
    ...
    logger.info("🔐 Probando encriptación de datos...")
    encrypted = encryption.encrypt_data(test_data)
    logger.info(f"✅ Datos encriptados: {len(encrypted['encrypted_data'])} caracteres")
    # Desencriptar datos
    decrypted = encryption.decrypt_data(encrypted)
    logger.info(f"✅ Datos desencriptados: {decrypted}")
    # Verificar integridad
    integrity_ok = encryption.verify_encryption_integrity(encrypted)
    logger.error(f"✅ Integridad verificada: {'✅' if integrity_ok else '❌'}")
    ...
    logger.info(f"✅ Archivo encriptado: {encrypted_file}")
    # Desencriptar archivo
    decrypted_file = encryption.decrypt_file(encrypted_file)
    logger.info(f"✅ Archivo desencriptado: {decrypted_file}")
...
async def main():
    ...
    logger.info("🔐 Sistema Unificado de Autenticación y Seguridad")
    logger.info("=" * 50)
    # Inicializar sistema
    ...
    logger.info(f"✅ Token JWT generado: {token[:20]}...")
    # Validar token
    payload = auth_system.validate_jwt_token(token)
    logger.info(f"✅ Token validado para usuario: {payload['user_id']}")
    # Generar contraseña segura
    secure_password = auth_system.generate_secure_password()
    logger.info(f"✅ Contraseña segura generada: {secure_password}")
    # Validar contraseña
    validation = auth_system.validate_password(secure_password)
    logger.info(f"✅ Validación de contraseña: {validation['valid']}")
    # Firmar datos
    data = "Datos importantes para firmar"
    signature = auth_system.sign_data(data)
    logger.info(f"✅ Firma digital generada: {signature[:20]}...")
    # Verificar firma
    is_valid = auth_system.verify_signature(data, signature)
    logger.info(f"✅ Firma verificada: {is_valid}")
