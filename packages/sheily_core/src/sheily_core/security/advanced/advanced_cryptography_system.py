...
class AdvancedCryptographySystem:
    ...
    def hybrid_encrypt(
        ...
        try:
            ...
            cipher = Cipher(
                algorithms.AES(aes_key), modes.CBC(iv), backend=self.backend
            )
            ...
            if not isinstance(public_key, rsa.RSAPublicKey):
                raise TypeError(
                    "La clave pública debe ser RSA para encriptar la clave AES."
                )
            ...
            if not isinstance(public_key, rsa.RSAPublicKey):
                raise TypeError(
                    "La clave pública debe ser RSA para encriptar la clave HMAC."
                )
    ...
    def hybrid_decrypt(
        self, encrypted_package: Dict[str, str], private_key_pem: bytes
    ) -> bytes:
        ...
        try:
            ...
            encrypted_hmac_key = base64.b64decode(
                encrypted_package["encrypted_hmac_key"]
            )
    ...
    def fast_encrypt(
        self, data: Union[str, bytes], key: bytes = None
    ) -> Dict[str, str]:
    ...
    def cleanup_session_keys(self):
        ...
        if expired_sessions:
            self.logger.info(
                f"Limpiadas {len(expired_sessions)} claves de sesión expiradas"
            )
...
if __name__ == "__main__":
    ...
    # Encriptar datos
    test_data = (
        "¡Este es un mensaje ultra secreto con encriptación híbrida RSA-4096 + AES-256!"
    )
    ...
    logger.info("✅ Sistema de Criptografía Avanzada funcionando correctamente")
    logger.info(f"Original: {test_data}")
    logger.info(f"Desencriptado: {decrypted.decode('utf-8')}")
    logger.info(f"Estado del sistema: {crypto.get_system_status()}")
