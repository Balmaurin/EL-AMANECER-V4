...
ALLOW = json.loads(
    (
        Path(os.getenv("SHEILY_MODELS_ALLOWLIST_PATH", "models/ALLOWLIST.json"))
    ).read_text("utf-8")
)
...
def precommit_check(repo_root: Path) -> int:
    ...
    logger.info("🎉 Pre-commit check: DESACTIVADO - Sin restricciones")
    return 0
...
def main():
    ...
    if args.cmd == "scan":
        logger.info(
            json.dumps(scan_models_dir(models_root), indent=2, ensure_ascii=False)
        )
        return
    if args.cmd == "enforce":
        rep = scan_models_dir(models_root)
        moved = quarantine(rep["blocked"], Path(args.quarantine))
        logger.info(json.dumps({"quarantined": moved}, indent=2, ensure_ascii=False))
        return
    if args.cmd == "precommit":
        sys.exit(precommit_check(Path.cwd()))
    if args.cmd == "install-hooks":
        path = install_precommit(Path.cwd())
        logger.info(json.dumps({"installed": path}, ensure_ascii=False))
        return
