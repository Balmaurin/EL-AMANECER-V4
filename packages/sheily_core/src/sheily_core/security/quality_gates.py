...
class QualityGateValidator:
    ...
    def _load_thresholds(self) -> Dict:
        ...
        if not self.thresholds_file.exists():
            raise FileNotFoundError(
                f"Archivo de umbrales no encontrado: {self.thresholds_file}"
            )
    ...
    def validate_report(self, report_data: Dict) -> Dict:
        ...
        total_validations = (
            ...
            + (1 if general_results else 0)
            + (1 if rag_results else 0)
        )
    ...
    def _validate_general_metrics(self, report_data: Dict) -> Dict:
        ...
        if "total_execution_time" in general_thresholds:
            ...
            actual = report_data.get("execution_info", {}).get(
    ...
    def _validate_rag_metrics(self, report_data: Dict) -> Optional[Dict]:
        ...
        for result in report_data.get("detailed_results", []):
            if result.get("test_name") == "rag_complete_suite" and result.get(
                "details"
            ):
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        if not self.violations:
            ...
            recommendations.append(
                "🎯 El sistema cumple con los estándares de calidad establecidos"
            )
        ...
        medium_violations = [
            v for v in self.violations if v.get("severity") == "medium"
        ]
        ...
        if high_violations:
            recommendations.append(
                "⚠️  ALTO: Violaciones importantes que requieren atención"
            )
        ...
        if medium_violations:
            recommendations.append(
                "📋 MEDIO: Mejoras recomendadas para optimizar calidad"
            )

        # Recomendaciones específicas
        if any(v.get("metric") == "success_rate" for v in self.violations):
            recommendations.append(
                "🔧 Revisar tests fallidos y corregir problemas de código"
            )
        ...
        if any("rag" in v.get("category", "") for v in self.violations):
            recommendations.append(
                "🤖 Revisar configuración y parámetros del sistema RAG"
            )
    ...
    def _print_validation_summary(self, quality_report: Dict):
        ...
        logger.error(
            f"   Estado: {'✅ PASADO' if summary['all_gates_passed'] else '❌ FALLIDO'}"
        )
        logger.info(f"   Puntuación: {summary['quality_score']:.1f}/100")
        logger.info(f"   Violaciones: {summary['violations_count']}")
        if self.violations:
            ...
            if len(self.violations) > 5:
                logger.info(f"   ... y {len(self.violations) - 5} violaciones más")
        logger.info(f"\n📊 RECOMENDACIONES:")
...
def validate_quality_gates(report_path: Path) -> bool:
    ...
    if not report_path.exists():
        logger.error(f"❌ Archivo de reporte no encontrado: {report_path}")
        return False
    ...
    try:
        ...
        logger.error(f"❌ Error validando quality gates: {e}")
        return False
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        logger.info("Uso: python quality_gates.py <ruta_reporte.json>")
        sys.exit(1)
