...
def test_single_prompt_similarity(python_emb, cpp_emb, tokens, prompt):
    ...
    logger.info("pytorch embeddings:")
    logger.info(python_emb)
    logger.info("llama.cpp embeddings:")
    logger.info(cpp_emb)
    logger.info(f"\n=== Prompt: '{prompt}' ===")
    logger.info(f"Tokens: {tokens}")
    logger.info(
        f"Embeddings shape: Python {python_emb.shape}, llama.cpp {cpp_emb.shape}"
    )
    n_tokens = len(tokens)
    ...
    if is_pooled:
        ...
        logger.info("   Python model:")
        for i in range(n_tokens):
            for j in range(i + 1, n_tokens):
                sim = cosine_similarity([python_emb[i]], [python_emb[j]])[0][0]
                logger.info(f"     {tokens[i]} ↔ {tokens[j]}: {sim:.4f}")
        logger.info("   llama.cpp model:")
        for i in range(n_tokens):
            for j in range(i + 1, n_tokens):
        ...
        for i in range(n_tokens):
            ...
            logger.info(f"   Token {i} ({tokens[i]}): {sim:.4f}")
        ...
        logger.info(f"   Max difference: {np.max(diff_matrix):.4f}")
        logger.info(f"   Mean difference: {np.mean(diff_matrix):.4f}")
        logger.info(f"   RMS difference: {np.sqrt(np.mean(diff_matrix**2)):.4f}")
        return {
            "cross_model_similarities": [
                cosine_similarity([python_emb[i]], [cpp_emb[i]])[0][0]
                for i in range(n_tokens)
            ],
...
def read_prompt_from_file(file_path):
    try:
        ...
        logger.error(f"Error: Prompts file '{file_path}' not found")
        exit(1)
...
def main():
    ...
    parser.add_argument(
        "--python-embeddings",
        "-pe",
        help='Path to pytorch embeddings "logits" binary file',
    )
    parser.add_argument(
        "--cpp-embeddings",
        "-ce",
        help='Path to llama.cpp embeddings "logits" binary file',
    )
    parser.add_argument(
        "--causal",
        "-c",
        default=False,
        help="if the model is causal (default: false)",
        action="store_true",
    )
    parser.add_argument(
        "--prompt", "-p", default="Hello world today", help="Test prompt"
    )
    ...
    logger.info(
        "Semantic Similarity Test Between Python and llama.cpp Embedding Models"
    )
    logger.info("=" * 70)
    # Single prompt detailed comparison
    logger.info(f"\nTesting with prompt: '{prompt}'")
    # Load the python model to get configuration information and also to load the tokenizer.
    logger.info("Loading model and tokenizer using AutoTokenizer:", args.model_path)
    ...
    if unreleased_model_name:
        ...
        logger.info(f"Model class: {class_name}")
        logger.info(f"Importing unreleased model module: {unreleased_module_path}")
        try:
            model_class = getattr(
            ...
            logger.error(f"Failed to import or load model: {e}")
            exit(1)
    else:
        if args.causal:
    ...
    # Run comparison
    results = test_single_prompt_similarity(
        python_embeddings, llamacpp_embeddings, tokens, prompt
    )

    # Summary
    logger.info(f"\n=== SUMMARY ===")
    avg_cross_sim = np.mean(results["cross_model_similarities"])
    logger.info(f"Average cross-model similarity: {avg_cross_sim:.4f}")
    logger.info(f"Similarity matrix RMS difference: {results['rms_diff']:.4f}")
    # Quality assessment
    if avg_cross_sim > 0.95:
        logger.info("✅ EXCELLENT: Models are highly similar")
    elif avg_cross_sim > 0.90:
        logger.info("✅ VERY GOOD: Models are very similar")
    elif avg_cross_sim > 0.80:
        logger.warning("⚠️  GOOD: Models are reasonably similar")
    elif avg_cross_sim > 0.70:
        logger.warning("⚠️  FAIR: Models have some differences")
    else:
        logger.error("❌ POOR: Models are significantly different")
