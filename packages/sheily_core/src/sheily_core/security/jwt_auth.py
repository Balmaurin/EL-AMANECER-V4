...
class JWTAuthManager:
    ...
    def __init__(
        ...
        if not self.secret_key or self.secret_key == "change_this_in_production":
            raise ValueError(
                "SECRET_KEY no configurada o usa valor por defecto. "
    ...
    def generate_token(
        self, user_id: str, role: str = "user", additional_claims: Optional[Dict] = None
    ) -> str:
        """
        Generar token JWT
    ...
    def check_permission(self, payload: TokenPayload, required_role: str) -> bool:
        ...
        if not has_permission:
            logger.warning(
                f"Permiso denegado: user_id={payload.user_id}, "
                f"role={payload.role}, required={required_role}"
            )
        ...
        def wrapper(token: str, *args, **kwargs):
            ...
            if not jwt_manager.check_permission(payload, required_role):
                raise PermissionError(
                    f"Rol insuficiente. Requerido: {required_role}, "
                    f"actual: {payload.role}"
                )
