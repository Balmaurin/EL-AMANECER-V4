...
def main(args_in: list[str] | None = None) -> None:
    ...
    try:
        ...
        logger.error("bench: server start error :")
    ...
    try:
        ...
        with open("results.github.env", "w") as github_env:
            # parse output
            with open("k6-results.json", "r") as bench_results_file:
                # Load JSON data from file
                data = json.load(bench_results_file)
    ...
    if server_process:
        try:
            logger.info(f"bench: shutting down server pid={server_process.pid} ...")
            if os.name == "nt":
    ...
    if is_server_listening("0.0.0.0", 9090):
        ...
        for metric in metrics:
            ...
            if resp.status_code != 200:
                ...
                timestamps_t, metric_values_t = zip(*values)
                metric_values: list[float] = [float(value) for value in metric_values_t]
                prometheus_metrics[metric] = metric_values
                timestamps_dt = [
                    str(datetime.fromtimestamp(int(ts))) for ts in timestamps_t
                ]
...
xychart-beta
    ...
    x-axis "llamacpp:{metric}" {int(min(timestamps_t))} --> {int(max(timestamps_t))}
    ...
    bench_results = {
        ...
        "pp": {
            ...
            "0": (
                round(mean(prometheus_metrics["prompt_tokens_seconds"]), 2)
        ...
        "tg": {
            ...
            "0": (
                round(mean(prometheus_metrics["predicted_tokens_seconds"]), 2)
...
def start_benchmark(args):
    ...
    # Construir comando como lista (más seguro que usar shell en True)
    k6_cmd = [str(k6_path)] + [str(arg) for arg in k6_args]
    logger.info(f"bench: starting k6 with: {' '.join(k6_cmd)}")
    k6_completed = subprocess.run(
        k6_cmd, env=k6_env, stdout=sys.stdout, stderr=sys.stderr
    )
...
def start_server(args):
    ...
    while not is_server_listening(args.host, args.port):
        ...
        logger.info("bench:     waiting for server to start ...")
        time.sleep(0.5)
    ...
    while not is_server_ready(args.host, args.port):
        ...
        logger.info("bench:     waiting for server to be ready ...")
        time.sleep(0.5)

    logger.info("bench: server started and ready.")
...
def start_server_background(args):
    ...
    logger.info(f"bench: starting server with: {' '.join(args)}")
    ...
    def server_log(in_stream, out_stream):
        for line in iter(in_stream.readline, b""):
            logger.info(line.decode("utf-8"), end="", file=out_stream)
...
def is_server_listening(server_fqdn, server_port):
    with closing(socket.socket(socket.AF_INET, socket.SOCK_STREAM)) as sock:
        ...
        if _is_server_listening:
            logger.info(f"server is listening on {server_fqdn}:{server_port}...")
