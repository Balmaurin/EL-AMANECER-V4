...
def resolve_package_dependency(
...
def compose_dependency_contexts(
    ...
    merged_allowed = list(
        set(base_context.allowed_packages + overlay_context.allowed_packages)
    )

    # Merge blocked packages
    merged_blocked = list(
        set(base_context.blocked_packages + overlay_context.blocked_packages)
    )
    ...
    merged_security = {
        **base_context.security_config,
        **overlay_context.security_config,
    }
...
def create_dependency_resolver() -> (
    ...
    def resolver(
        ...
        resolution = create_dependency_resolution(
            ...
            resolved_package=resolved_package,
            resolution_method=(
                "direct" if package_name == resolved_package else "mapped"
            ),
            ...
            metadata={
                ...
                "security_level": context.security_config.get(
...
def create_safe_import_executor() -> (
    Callable[[str, DependencyContext], Result[Any, str]]
):
...
def resolve_dependency_functional(
    package_name: str, training_mode: bool = False
) -> Dict[str, Any]:
