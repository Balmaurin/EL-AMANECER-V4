...
class RealMergerAnalyzer:
    ...
    def analyze_integration_context(
        ...
        result = {
            ...
            "specialized_coverage": round(
                len(common_query_spec) / max(len(query_words), 1), 3
            ),
            "additional_info": general if complementary else None,
        }

        logger.debug(
            f"Integration analysis: {len(integration_points)} points, score={integration_score:.3f}"
        )
        return result
    ...
    def analyze_consensus(self, sources: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        result = {
            ...
            "agreement_score": round(
                avg_similarity if similarity_scores else avg_confidence, 3
            ),
        ...
        logger.debug(
            f"Consensus analysis: level={consensus_level:.3f}, sources={len(sources)}"
        )
        return result

    def build_consensus_response(
        self, sources: List[Dict[str, Any]], analysis: Dict[str, Any]
    ) -> str:
        """
    ...
    def evaluate_merge_characteristics(self, merge_input: Any) -> Dict[str, Any]:
        ...
        complexity_indicators = [
            ...
            any(
                keyword in spec_text.lower()
                for keyword in ["sin embargo", "por otro lado", "además", "también"]
            ),
        ...
        logger.debug(
            f"Merge characteristics: complexity={complexity_high}, sources={len(additional)}"
        )
        return result

    def analyze_branch_complementarity(self, responses: List[Any]) -> Dict[str, Any]:
        """
    ...
    def assess_multi_branch_quality(
        ...
        logger.debug(
            f"Quality assessment: {quality_score:.3f} (length={length_score:.2f}, diversity={diversity_score:.2f if coverage_scores else 0:.2f})"
        )
    ...
    def _find_context(
        self, keyword: str, text: str, context_size: int = 100
    ) -> Optional[str]:
...
# Exports
__all__ = ["RealMergerAnalyzer", "AnalysisResult", "get_real_analyzer"]
