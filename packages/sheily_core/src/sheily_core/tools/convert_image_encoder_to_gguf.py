...
def get_non_negative_vision_feature_layers(v_hparams):
    ...
    to_non_negative = lambda layer_idx: (
        layer_idx if layer_idx >= 0 else num_hidden_layers + layer_idx + 1
    )
...
if has_llava_projector:
    ...
    logger.info("Projector tensors added\n")
...
for name, data in state_dict.items():
    if should_skip_tensor(
        ...
        # we don't need this
        logger.info(f"skipping parameter: {name}")
        continue
    ...
    logger.info(f"{name} - {ftype_str[ftype_cur]} - shape = {data.shape}")
    fout.add_tensor(name, data)
...
logger.info("Done. Output file: " + fname_out)
