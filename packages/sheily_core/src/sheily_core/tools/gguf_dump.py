...
def dump_markdown_metadata(reader: GGUFReader, args: argparse.Namespace) -> None:
    ...
    for n, field in enumerate(reader.fields.values(), 1):
        ...
        def escape_markdown_inline_code(value_string):
            ...
            max_backticks = max(
                (len(match.group(0)) for match in re.finditer(r"`+", value_string)),
                default=0,
            )
        ...
        if len(field.types) == 1:
            ...
            if curr_type == GGUFValueType.STRING:
                ...
                if len(value_string) > truncate_length:
                    head = escape_markdown_inline_code(
            ...
            if field.types[0] == GGUFValueType.ARRAY:
                ...
                if curr_type == GGUFValueType.STRING:
                    ...
                    for element_pos in range(render_element):
                        ...
                        value_string = str(
                            bytes(
                                field.parts[-1 - (total_elements - element_pos - 1) * 2]
                            ),
                            encoding="utf-8",
                        )
                        if len(value_string) > truncate_length:
    ...
    if not args.no_tensors:
        ...
        for key, tensor in enumerate(reader.tensors):
            ...
            if tensor_components[0] == "blk":
                ...
                tensor_components[0] in ["enc", "dec"] and tensor_components[1] == "blk"
            ):
        ...
        markdown_content += (
            f"Total number of elements in all tensors: {total_elements} Elements\n"
        )
        markdown_content += "\n"
        ...
        for group in tensor_prefix_order:
            ...
            for tensor in tensors:
                ...
                for i, dimension_size in enumerate(
                    ...
                    prettify_dimension_max_widths[i] = max(
                        prettify_dimension_max_widths.get(i, 1),
            ...
            tensor_dump_table_header_map = [
                ...
                },
                {
                    ...
                    "header_name": "Human Friendly Tensor Layer Name",
                    "align": "left",
                },
                {
            ...
            markdown_content += (
                f"- Percentage of total elements: {group_percentage:.2f}%\n"
            )
    ...
    if total_model_elements > 0:
        ...
        markdown_content += (
            f"Total BPW for {os.path.basename(args.model)}: undefined (no elements)"
        )
...
def main() -> None:
    ...
    parser.add_argument(
        "--data-offset", action="store_true", help="Start of data offset"
    )
    parser.add_argument(
        "--data-alignment",
        action="store_true",
        help="Data alignment applied globally to data field",
    )
    parser.add_argument(
        "--markdown", action="store_true", help="Produce markdown output"
    )
    parser.add_argument(
    ...
    if (
        ...
        and not args.data_offset
        and not args.data_alignment
    ):
