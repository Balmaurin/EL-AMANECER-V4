    ...
    if is_safetensor_file(file_path):
        ...
        with cast(
            ...
            for key in f.keys():
                ...
                # output shape
                logger.info(f"{key} : {tensors[key].shape}")
...
def clean_vision_tower_from_checkpoint(checkpoint_path):
    ...
    logger.info(f"Searching for vision tower tensors in {checkpoint_path}")
    clip_tensors = [k for k, v in checkpoint.items() if is_vision_tower(k)]

    if len(clip_tensors) > 0:
        logger.info(
            f"Found {len(clip_tensors)} tensors to extract from {checkpoint_path}"
        )
        # Adapted for file type
        clip_path = os.path.join(model_path, "llava.clip")

        if os.path.exists(clip_path):
            logger.info(f"Loading existing llava.clip from {clip_path}")
            existing_clip, _ = load_model(clip_path)
        else:
            logger.info(f"Creating new llava.clip at {clip_path}")
            existing_clip = {}
        ...
        for name in clip_tensors:
            ...
            logger.info(f"Adding {simple_name} to llava.clip")
...
if args.clean_vision_tower:
    ...
    for projector_checkpoint_path in checkpoint_paths:
        logger.info(f"Cleaning {projector_checkpoint_path}")
        if not clean_vision_tower_from_checkpoint(projector_checkpoint_path):
            logger.info(f"No vision tower found in {projector_checkpoint_path}")
...
logger.info(f"Taking projector from {projector_checkpoint_path}")
first_mm_tensors = []
first_checkpoint = None
if newline_checkpoint_path is not None:
    logger.info(f"Taking newline from {newline_checkpoint_path}")
...
if len(mm_tensors) == 0:
    if last_checkpoint is not None:
        for k, v in last_checkpoint.items():
            logger.info(k)
    mm_count = len(mm_tensors)
    checkpoint_count = len(last_checkpoint) if last_checkpoint is not None else 0
    print(
        f"Found {mm_count} tensors to extract out of {checkpoint_count} tensors."
    )  # noqa: T201
    logger.info("No tensors found. Is this a LLaVA model?")
    exit()

logger.info(f"Found {len(mm_tensors)} tensors to extract.")
logger.info(f"Found additional {len(first_mm_tensors)} tensors to extract.")
...
logger.info("Done!")
logger.info(f"Now you can convert {args.model} to a regular LLaMA GGUF file.")
logger.info(
    f"Also, use {args.model}/llava.projector to prepare a llava-encoder.gguf file."
)
