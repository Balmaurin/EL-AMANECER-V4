...
def main():
    ...
    ap.add_argument(
        "--min_chars", type=int, default=200, help="Mínimo de caracteres por ejemplo"
    )
    args = ap.parse_args()
    ...
    with args.out.open("w", encoding="utf-8") as f:
        for rama, path in iter_corpus_files(args.branch):
            try:
                ...
                logger.warning(f"WARNING: No se pudo leer {path}: {e}")
                continue
    ...
    logger.info(f"✅ Dataset generado: {args.out} ({count} ejemplos)")
