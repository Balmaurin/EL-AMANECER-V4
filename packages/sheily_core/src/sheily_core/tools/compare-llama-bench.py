...
TEST_BACKEND_OPS_KEY_PROPERTIES = ["backend_name", "op_name", "op_params", "test_mode"]
...
LLAMA_BENCH_PRETTY_NAMES = {
    ...
    "tensor_buft_overrides": "Tensor overrides",
    "model_filename": "File",
    "model_type": "Model",
...
parser.add_argument(
    ...
    help=help_t,
    default=None,
    choices=[None, "llama-bench", "test-backend-ops"],
)
help_i = (
...
parser.add_argument(
    "--plot_x",
    help="parameter to use as x axis for plotting (default: n_depth)",
    default="n_depth",
)
parser.add_argument(
    "--plot_log_scale",
    action="store_true",
    help="use log scale for x axis in plots (off by default)",
)
...
class LlamaBenchData:
    ...
    def __init__(self, tool: str = "llama-bench"):
        ...
        if self.tool == "llama-bench":
            ...
            self.check_keys = set(
                TEST_BACKEND_OPS_KEY_PROPERTIES + ["build_commit", "test_time"]
            )
        else:
            assert False
    ...
    def builds_timestamp(
        self, reverse: bool = False
    ) -> Union[Iterator[tuple], Sequence[tuple]]:
...
class LlamaBenchDataSQLite3(LlamaBenchData):
    ...
    def __init__(self, tool: str = "llama-bench"):
        ...
        if self.connection is None:
            ...
            self.cursor.execute(
                f"CREATE TABLE {self.table_name}({', '.join(' '.join(x) for x in zip(db_fields, db_types))});"
            )

    def _builds_init(self):
        if self.connection:
            self.build_len_min = self.cursor.execute(
                f"SELECT MIN(LENGTH(build_commit)) from {self.table_name};"
            ).fetchone()[0]
            self.build_len_max = self.cursor.execute(
                f"SELECT MAX(LENGTH(build_commit)) from {self.table_name};"
            ).fetchone()[0]

            if self.build_len_min != self.build_len_max:
                logger.warning(
                    "Data contains commit hashes of differing lengths. It's possible that the wrong commits will be compared. "
    ...
    def builds_timestamp(
        ...
        data = self.cursor.execute(
            f"SELECT build_commit, test_time FROM {self.table_name} ORDER BY test_time;"
        ).fetchall()
    ...
    def get_rows(
        ...
        if self.tool == "llama-bench":
            return self._get_rows_llama_bench(
                properties, hexsha8_baseline, hexsha8_compare
            )
        elif self.tool == "test-backend-ops":
            return self._get_rows_test_backend_ops(
                properties, hexsha8_baseline, hexsha8_compare
            )
        else:
            assert False

    def _get_rows_llama_bench(
        self, properties: list[str], hexsha8_baseline: str, hexsha8_compare: str
    ) -> Sequence[tuple]:
        select_string = ", ".join(
    ...
    def _get_rows_test_backend_ops(
        self, properties: list[str], hexsha8_baseline: str, hexsha8_compare: str
    ) -> Sequence[tuple]:
        ...
        select_string = ", ".join(
            ...
            + [
                "AVG(tb.flops)",
                "AVG(tc.flops)",
                "AVG(tb.bandwidth_gb_s)",
                "AVG(tc.bandwidth_gb_s)",
            ]
        )
        equal_string = " AND ".join(
    ...
    def __init__(self, data_file: str, tool: Any):
        ...
        if tool is None:
            ...
            if "test_backend_ops" in table_names:
                ...
                raise RuntimeError(
                    f"Table 'test_backend_ops' not found for tool 'test-backend-ops'. Available tables: {table_names}"
                )
        else:
        ...
        try:
            if cursor.execute("PRAGMA schema_version;").fetchone()[0] == 0:
                raise sqlite3.DatabaseError(
                    "The provided input file does not exist or is empty."
                )
...
class LlamaBenchDataJSONL(LlamaBenchDataSQLite3):
    def __init__(self, data_file: str, tool: str = "llama-bench"):
        ...
        db_fields = (
            LLAMA_BENCH_DB_FIELDS
            if tool == "llama-bench"
            else TEST_BACKEND_OPS_DB_FIELDS
        )
...
# Otherwise, search for the most recent parent of master for which there is data:
    ...
    hexsha8_baseline = bench_data.find_parent_in_data(
        bench_data.repo.heads.master.commit
    )
...
# and that is not a parent of master:
    ...
    hexsha8s_master = bench_data.get_all_parent_hexsha8s(
        bench_data.repo.heads.master.commit
    )
...
if known_args.show is not None:
    ...
    for prop in show:
        valid_props = (
            key_properties if tool == "test-backend-ops" else key_properties[:-3]
        )  # Exclude n_prompt, n_gen, n_depth for llama-bench
...
if not rows_show:
    logger.error(
        f"No comparable data was found between {name_baseline} and {name_compare}.\n"
    )
    sys.exit(1)
...
if tool == "llama-bench":
    ...
    headers += [
        f"{primary_metric} {name_baseline}",
        f"{primary_metric} {name_compare}",
        "Speedup",
    ]
else:
    assert False
...
if known_args.plot:
    ...
    def create_performance_plot(
        ...
        if plot_x_param not in ["n_prompt", "n_gen", "n_depth"]:
            ...
            if pretty_name in data_headers:
                ...
                logger.error(
                    f"Parameter '{plot_x_param}' not found in current table columns. Available columns: {', '.join(data_headers)}"
                )
                return
        ...
        def make_axes(num_groups, max_cols=2, base_size=(8, 4)):
            ...
            fig, ax_arr = plt.subplots(
                rows, cols, figsize=(w * cols, h * rows), squeeze=False
            )
        ...
        for group_key, points in grouped_data.items():
            ...
            try:
                points_sorted = sorted(
                    points,
                    key=lambda p: (
                        float(p["x_value"]) if p["x_value"] is not None else 0
                    ),
                )
                x_values = [
                    float(p["x_value"]) if p["x_value"] is not None else 0
        ...
        fig.suptitle(
            f"Performance comparison: {compare_name} vs. {baseline_name}",
            fontsize=14,
            fontweight="bold",
        )
    ...
    create_performance_plot(
        ...
        known_args.plot,
        known_args.plot_x,
        known_args.plot_log_scale,
        tool,
        primary_metric,
    )

print(
    tabulate(table, headers=headers, floatfmt=".2f", tablefmt=known_args.output)
)  # noqa: NP100
