...
class BranchMerger:
    ...
    def __init__(self, config: Dict):
        ...
        """
        self.config = config
        self.default_strategy = MergeStrategy(
            config.get("default_strategy", "adaptive_merge")
        )
        ...
        self.enable_consensus_validation = config.get(
    ...
    async def merge_responses(
        ...
        try:
            ...
            if self.cache_enabled and cache_key in self.merge_cache:
                ...
                logger.debug(
                    f"Cache hit para fusión de {specialized_response.branch_name}"
                )
            ...
            if quality_score < self.quality_threshold:
                ...
                alternative_strategy = await self._select_alternative_strategy(
                    merge_strategy
                )
                merged_result = await self._execute_merge_strategy(
                    merge_input, alternative_strategy
                )
                quality_score = await self._assess_merge_quality(
                    merged_result, merge_input
                )
            ...
            await self._update_merge_stats(
                final_result, merge_strategy, processing_time
            )
    ...
    async def merge_multiple_branches(
        ...
        try:
            ...
            await self._update_merge_stats(
                result, MergeStrategy.CONSENSUS_BASED, processing_time
            )
    ...
    async def _extract_general_knowledge(
        self, query: str, specialized_response: Any
    ) -> str:
        """
    ...
    async def _weighted_combination_merge(
        self, merge_input: MergeInput
    ) -> Dict[str, Any]:
    ...
    async def _hierarchical_merge(self, merge_input: MergeInput) -> Dict[str, Any]:
        ...
        sources.extend(
            [s.get("name", "additional") for s in merge_input.additional_sources]
        )
    ...
    async def _execute_multi_branch_fusion(
        ...
        sorted_responses = sorted(
            branch_responses, key=lambda r: r.confidence, reverse=True
        )

        # Análisis de complementariedad
        complementarity_analysis = await self._analyze_branch_complementarity(
            sorted_responses
        )
        ...
        quality_score = await self._assess_multi_branch_quality(
            fused_response, sorted_responses
        )
    ...
    async def _generate_contextual_introduction(
        self, query: str, branch_name: str
    ) -> str:
    ...
    async def _generate_integrative_conclusion(
        self, specialized_response: Any, query: str
    ) -> str:
    ...
    async def _select_alternative_strategy(
        self, current_strategy: MergeStrategy
    ) -> MergeStrategy:
    ...
    async def _assess_merge_quality(
        self, merged_result: Dict, merge_input: MergeInput
    ) -> float:
    ...
    async def _analyze_consensus(self, sources: List[Dict]) -> Dict:
        ...
        return {
            ...
            "agreement_level": (
                ...
                else "medium" if consensus_level > 0.4 else "low"
            ),
            "conflicts": (
                [] if result["divergence"] < 0.3 else ["Divergencia detectada"]
            ),
            "num_sources": result["num_sources"],
        }

    async def _build_consensus_response(
        self, sources: List[Dict], analysis: Dict
    ) -> str:
    ...
    async def _evaluate_merge_characteristics(self, merge_input: MergeInput) -> Dict:
        ...
        # Añadir context_rich
        result["context_rich"] = bool(
            merge_input.merge_context and merge_input.general_knowledge
        )
        return result

    async def _apply_consensus_refinement(
        self, result: Dict, merge_input: MergeInput
    ) -> Dict:
        ...
        quality_score = analyzer.assess_multi_branch_quality(
            response_text, original_responses
        )
    ...
    async def _apply_adaptive_improvements(
        self, result: Dict, merge_input: MergeInput
    ) -> Dict:
    ...
    async def _analyze_branch_complementarity(self, responses: List[Any]) -> Dict:
        ...
        result["overlap_areas"] = (
            ["overlap detectado"] if result["overlap_percentage"] > 0.3 else []
        )
        result["unique_contributions"] = (
            [f"Contribución de {r.branch_name}" for r in responses]
            if hasattr(responses[0], "branch_name")
            else []
        )

        return result

    async def _create_complementary_fusion(
        self, responses: List[Any], analysis: Dict
    ) -> str:
        ...
        if high_complementarity:
            fusion_parts.append(
                "**Análisis Multi-Dominio Complementario** (Alta Complementariedad)\n"
            )
        else:
        ...
        for i, response in enumerate(responses[:3], 1):
            ...
            unique_contribution = (
                f" (Conceptos clave: {', '.join(keywords[:3])})" if keywords else ""
            )
