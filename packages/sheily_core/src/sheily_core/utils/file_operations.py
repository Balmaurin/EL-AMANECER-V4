...
class FileOperations:
    ...
    def handle_upload_document(
        self, post_data: bytes, rag_pipeline=None
    ) -> Dict[str, Any]:
    ...
    def handle_delete_document(self, doc_id: str, rag_pipeline=None) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "deleted_file": doc_id,
                "documents_remaining": (
                    len(rag_pipeline.documents) if rag_pipeline else 0
                ),
            }
        ...
        try:
            ...
            for doc in documents:
                try:
                    ...
                    result = self._upload_single_document(
                        filename, content, category, overwrite
                    )
    ...
    def handle_export_corpus(
        ...
        try:
            ...
            export_result = self._export_corpus_data(
                export_format, include_metadata, rag_pipeline
            )
    ...
    def handle_reindex(self, rag_pipeline=None) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "vocabulary_size": (
                    len(rag_pipeline.embedder.vocabulary)
    ...
    def _find_document_path(self, doc_id: str, rag_pipeline=None) -> str:
        ...
        for doc in rag_pipeline.documents:
            if (
                isinstance(doc, dict)
                and os.path.basename(doc.get("path", "")) == doc_id
            ):
                return doc["path"]
        return ""
    ...
    def _replace_section(
        ...
        if start_index != -1 and end_index != -1 and end_index > start_index:
            return (
                content[:start_index]
                + new_section
                + content[end_index + len(section_end) :]
            )
        else:
            return content + "\n\n" + new_section
    ...
    def _export_corpus_data(
        ...
        for doc in rag_pipeline.documents:
            ...
            if include_metadata:
                doc_entry.update(
                    {"word_count": len(str(doc).split()), "char_count": len(str(doc))}
                )
    ...
    def _export_embeddings_data(self, export_format: str, rag_pipeline=None) -> Dict:
        ...
        embeddings_info = {
            ...
            "embeddings_available": bool(
                rag_pipeline and hasattr(rag_pipeline, "embedder")
            ),
        ...
        if rag_pipeline and hasattr(rag_pipeline.embedder, "vocabulary"):
            embeddings_info.update(
                {
                    "vocabulary_size": len(
                        getattr(rag_pipeline.embedder, "vocabulary", {})
                    ),
