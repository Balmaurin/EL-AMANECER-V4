...
class ServerProcess:
    ...
    def start(self, timeout_seconds: int | None = DEFAULT_HTTP_TIMEOUT) -> None:
        ...
        logger.info(f"tests: starting server with: {' '.join(args)}")
        flags = 0
        if "nt" == os.name:
        ...
        logger.info(f"server pid={self.process.pid}, pytest pid={os.getpid()}")
    ...
    def stop(self) -> None:
        if self.external_server:
            logger.info("[external_server]: Not stopping external server")
            return
    ...
    def make_request(
        ...
        if requests is None:
            raise ImportError(
                "requests module is not available. Please install it with: pip install requests"
            )
        ...
        logger.info("Response from server", json.dumps(result.body, indent=2))
        return result
    ...
    def make_stream_request(
        ...
        for line_bytes in response.iter_lines():
            ...
            if "[DONE]" in line:
                ...
                logger.info("Partial response from server", json.dumps(data, indent=2))
                yield data
    ...
    def make_any_request(
        ...
        if stream:
            ...
            for chunk in self.make_stream_request(method, path, data, headers):
                if chunk["choices"]:
                    ...
                    if choice["delta"].get("reasoning_content") is not None:
                        assert (
                            len(choice["delta"]["reasoning_content"]) > 0
            ...
            logger.info("Final response from server", json.dumps(result, indent=2))
            return result
        else:
...
def parallel_function_calls(
    ...
    if exceptions:
        ...
        for index, error in exceptions:
            logger.error(f"Function at index {index}: {error}")
    return results
...
def download_file(url: str, output_file_path: str | None = None) -> str:
    ...
    if wget is None:
        raise ImportError(
            "wget module is not available. Please install it with: pip install wget"
        )
    ...
    if not os.path.exists(output_file):
        ...
        logger.info(f"Done downloading to {output_file}")
    else:
