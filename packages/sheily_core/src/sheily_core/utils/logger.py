...
class SheilyLogger:
    ...
    def _setup_logger(self):
        ...
        self.logger.setLevel(
            getattr(logging, self.config.logging.log_level.upper(), logging.INFO)
        )
...
def log_performance(operation: str, duration: float, **metadata):
    ...
    logger.info(
        ...
        duration=duration,
        extra={"performance_data": metadata},
    )
...
def log_error(error: Exception, context: str = "", **metadata):
    ...
    logger = get_logger("main")
    logger.exception(
        f"Error in {context}: {str(error)}", extra={"error_metadata": metadata}
    )
...
def log_request(
    ...
    with logger.context(request_id=request_id, operation=f"{method} {endpoint}"):
        if status_code >= 400:
            logger.warning(
                f"HTTP request completed with status {status_code}", duration=duration
            )
        else:
