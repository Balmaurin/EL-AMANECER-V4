...
class RemoteTensor:
    ...
    def data(self) -> bytearray:
        """
        Obtener datos remotos con manejo de errores y reintentos.

        Returns:
            bytearray: Datos obtenidos del servidor remoto

        Raises:
            ConnectionError: Si no se puede conectar al servidor
            ValueError: Si los datos recibidos son inválidos
        """
        max_retries = 3
        retry_delay = 1.0
        ...
        for attempt in range(max_retries):
            try:
                ...
                raise ValueError(f"Error fetching remote data: {e}") from e
...
class SafetensorRemote:
    ...
    def get_data_by_range(cls, url: str, start: int, size: int = -1) -> bytes:
        ...
        try:
            ...
            raise ImportError(
                "requests library is required for remote data fetching"
            ) from e
        ...
        try:
            ...
            response = requests.get(
                url, allow_redirects=True, headers=headers, timeout=30
            )
    ...
    def check_file_exist(cls, url: str) -> bool:
        ...
        try:
            ...
            raise ImportError(
                "requests library is required for remote file checking"
            ) from e
