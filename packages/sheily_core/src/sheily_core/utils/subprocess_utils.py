...
def validate_command_args(args: List[str]) -> bool:
    ...
    """
    # Caracteres peligrosos que pueden permitir inyección SOLO si shell=True
    # Como SIEMPRE usamos shell=False, \n y \r son seguros
    dangerous_chars = [";", "|", "&", "$", "`", "||", "&&"]
    ...
    for arg in args:
        ...
        for char in dangerous_chars:
            if char in arg_str:
                raise ValueError(
                    f"Carácter potencialmente peligroso '{char}' detectado en: {arg_str[:50]}"
                )
...
def safe_subprocess_run(
    ...
    **kwargs,
    ...
    # NUNCA usar el parámetro shell establecido a True
    if kwargs.get("shell", False):
        logger.warning(
            "Intentando usar parámetro 'shell' a True - forzando a False por seguridad"
        )
        kwargs["shell"] = False
