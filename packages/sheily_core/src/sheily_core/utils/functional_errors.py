                        ...
                        if strategy.can_recover(error):
                            ...
                            if recovery_result.is_ok():
                                ...
                                return ContextualResult(
                                    ...
                                    warnings=[
                                        f"Recovered using {strategy.__class__.__name__}"
                                    ],
                                )
        ...
        if result.is_ok():
            try:
                ...
                error = SheilyError(
                    ...
                    severity=ErrorSeverity.HIGH,
                    context=ErrorContext(
                        component="async_pipeline", operation=func.__name__
                    ),
                    cause=e,
                )
                result = Err(error)
...
def bind_results(
    results: List[Result[T, SheilyError]],
) -> Result[List[T], List[SheilyError]]:
...
class ErrorMonitor:
    ...
    def record_error(self, error: SheilyError, recovery_time: float = 0.0):
        ...
        # Log del error
        self.logger.error(
            f"Error recorded in {component}",
            extra={"error": error, "recovery_time": recovery_time},
        )
    ...
    def get_error_summary(self) -> Dict[str, Any]:
        ...
        components_with_errors = len(
            [m for m in self.metrics.values() if m.total_errors > 0]
        )
        ...
        return {
            ...
            "most_problematic_component": (
                max(self.metrics.items(), key=lambda x: x[1].total_errors)[0]
                if self.metrics
                else None
            ),
            "error_rate_per_minute": sum(
                m.error_rate_per_minute for m in self.metrics.values()
            ),
...
def _categorize_exception(e: Exception) -> ErrorCategory:
    ...
    if any(
        ...
        word in error_msg for word in ["model", "inference", "llama", "tokenizer"]
    ):
        ...
        word in error_msg
        for word in ["auth", "authentication", "permission", "unauthorized"]
    ):
...
def _determine_severity(e: Exception) -> ErrorSeverity:
    ...
    if any(
        word in error_msg for word in ["critical", "fatal", "system", "out of memory"]
    ):
...
def create_validation_error(message: str, **metadata) -> SheilyError:
    ...
    return create_error(
        message, ErrorCategory.VALIDATION, ErrorSeverity.HIGH, **metadata
    )
...
def create_memory_error(message: str, **metadata) -> SheilyError:
    ...
    return create_error(
        message, ErrorCategory.MEMORY, ErrorSeverity.CRITICAL, **metadata
    )
...
if _os.environ.get("SHEILY_CHAT_QUIET", "1") != "1":
    logger.error(
        "✅ Sistema avanzado de manejo de errores funcionales cargado exitosamente"
    )
