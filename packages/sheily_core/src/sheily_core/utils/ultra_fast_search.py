...
class SearchIndex:
    ...
    content_index: Dict[str, List[str]] = field(
...
class UltraFastFileSearcher:
    ...
    async def build_index(self, force_rebuild: bool = False) -> Dict[str, Any]:
        ...
        logger.info(
            f"✅ Índice construido: {total_indexed} archivos en {build_time:.2f}s"
        )
            ...
            for root_dir in [self.root_path]:
                if root_dir.exists():
                    task = loop.run_in_executor(
                        executor, find_files_recursive, root_dir
                    )
        ...
        try:
            ...
            with self.index_lock:
                ...
                for word in words:
                    ...
                    self.index.word_frequency[word] = (
                        self.index.word_frequency.get(word, 0) + 1
                    )
    ...
    async def search(self, query: str, max_results: int = 20) -> List[SearchResult]:
        ...
        sorted_results = sorted(
            all_results.values(), key=lambda x: x.relevance_score, reverse=True
        )
            ...
            for similar_word in similar_words:
                if similar_word in self.index.content_index:
                    for file_path in self.index.content_index[similar_word]:
                        relevance = (
                            self._calculate_relevance(file_path, query, query_words)
                            * 0.7
                        )
                        if relevance > 0.05:
                            result = self._create_search_result(
                                file_path, relevance, query
                            )
                            results.append(result)
                ...
                for synonym in synonyms:
                    if synonym in self.index.content_index:
                        for file_path in self.index.content_index[synonym]:
                            relevance = (
                                0.3  # Baja relevancia para búsqueda semántica básica
                            )
                            result = self._create_search_result(
                                file_path, relevance, query
                            )
    ...
    def _find_similar_words(self, word: str) -> List[str]:
        ...
        for indexed_word in list(self.index.content_index.keys())[
            :1000
        ]:  # Limitar búsqueda
    ...
    def _calculate_relevance(
        self, file_path: str, query: str, query_words: List[str]
    ) -> float:
        ...
        for word in query_words:
            if (
                word in self.index.content_index
                and file_path in self.index.content_index[word]
            ):
                words_found += 1
    ...
    def _create_search_result(
        self, file_path: str, relevance: float, query: str
    ) -> SearchResult:
...
class SEILiCoreOptimizer:
    ...
    async def _process_context_parallel(
        ...
        sorted_results = sorted(
            filtered_results, key=lambda x: x["relevance"], reverse=True
        )
...
async def optimized_sei_response(
