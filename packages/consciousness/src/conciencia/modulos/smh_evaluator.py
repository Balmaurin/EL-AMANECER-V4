...
class SMHEvaluator:
    ...
    def _calculate_amygdala_input(self, stimulus: Dict[str, float]) -> float:
        ...
        threat_level = stimulus.get('threat', stimulus.get('prediction_error', 0.0) * 0.5)
        reward_level = stimulus.get('reward', 0.0)
        novelty = stimulus.get('novelty', 0.0)
        intensity = stimulus.get('intensity', max(stimulus.values()) if stimulus else 0.5)
        
        activation = (
            abs(threat_level) * 0.7 * max(0, intensity) +
            abs(reward_level) * 0.5 * max(0, intensity) +
            abs(novelty) * 0.3
        )
