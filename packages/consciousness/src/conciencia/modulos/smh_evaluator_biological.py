...
This implementation goes beyond simple computational analog to model:
...
class PhysiologicalState:
    ...
    adrenaline_level: float = 0.0      # pg/mL (baseline ~50)
...
# ============================================================================
# MAIN BIOLOGICAL SMH SYSTEM
# ============================================================================

class BiologicalSMHSystem:
    """
    Biologically realistic Somatic Marker Hypothesis implementation
    
    Models the complete physiological and neural processes, not just
    computational analogs. Includes:
    ...
    def __init__(self):
        ...
        self.markers: List[BiologicalSomaticMarker] = []
        ...
        self.consolidation_rate = 0.001  # Sleep-dependent consolidation
    ...
    def process_stimulus(self, 
        ...
        Timeline (biologically realistic):
        ...
        150-300ms: Autonomic response begins
        300-500ms: Full physiological response
        500-2000ms: Sustained marker state + decision bias
        
        Args:
            stimulus: Current sensory/cognitive input
            context: Situational context
            duration: How long to process (seconds)
            
        Returns:
            Complete biological response including physiology, neural, behavior
        """
        # Timeline
        ...
        for step in range(n_steps):
            ...
            # PHASE 1: AMYGDALA (fast threat/reward detection)
            if step * self.dt < 0.030:  # 0-30ms
            ...
            # PHASE 2: INSULA (interoceptive representation)
            if 0.030 <= step * self.dt < 0.080:  # 30-80ms
                # Insula represents current body state
            ...
            # PHASE 3: vmPFC/OFC (marker retrieval and evaluation)
            if 0.080 <= step * self.dt < 0.150:  # 80-150ms
                # Retrieve relevant markers
                relevant_markers = self._retrieve_markers_biological(
                    stimulus, context
                )
                
                # vmPFC reactivates stored marker patterns
                if relevant_markers:
            ...
            # PHASE 4: AUTONOMIC RESPONSE (150-300ms)
            if 0.150 <= step * self.dt < 0.300:
                # Markers trigger autonomic changes
                self._update_autonomic_from_markers(relevant_markers if 'relevant_markers' in locals() else [])
            
            # PHASE 5: FULL PHYSIOLOGICAL RESPONSE (300-500ms)
            if 0.300 <= step * self.dt:
                # Update complete physiological state
                self._update_physiology(self.dt)
                
                # Update neurotransmitters
                self._update_neurotransmitters(relevant_markers if 'relevant_markers' in locals() else [], self.dt)
            
            # Record state
            self._record_state()
            
            # Homeostatic regulation (continuous)
            self._homeostatic_regulation(self.dt)
        
        # Generate final response
        return self._generate_response(relevant_markers if 'relevant_markers' in locals() else [])
    ...
    def _calculate_amygdala_input(self, stimulus: Dict[str, float]) -> float:
        ...
        - Threat cues (negative valence, high intensity)
        - Reward cues (positive valence, high salience)
        - Novelty (low familiarity)
        """
        threat_level = stimulus.get('threat', 0.0)
        reward_level = stimulus.get('reward', 0.0)
        novelty = stimulus.get('novelty', 0.0)
        intensity = stimulus.get('intensity', 0.5)
        
        # Amygdala activation function
        activation = (
            threat_level * 0.7 * intensity +
            reward_level * 0.5 * intensity +
            novelty * 0.3
        )
        
        return np.clip(activation, 0, 1)
    
    def _calculate_insula_input(self, 
                                stimulus: Dict[str, float],
                                physiology: PhysiologicalState) -> float:
        """
        ...
        - Emotional context
        - Prediction errors about body state
        """
        # Current interoceptive state
        interoceptive_intensity = (
            abs(physiology.heart_rate - 70) / 30 * 0.3 +  # HR deviation
            physiology.skin_conductance_level / 20 * 0.3 +  # SCL
            abs(physiology.respiration_rate - 15) / 5 * 0.2 +  # Resp deviation
            physiology.muscle_tension * 0.2
        )
        
        # Prediction error (if stimulus suggests different body state than actual)
    ...
    def _retrieve_markers_biological(self,
                                    stimulus: Dict[str, float],
                                    context: str,
                                    threshold: float = 0.6) -> List[BiologicalSomaticMarker]:
        """
        Retrieve markers with biological constraints
        
        Not instant retrieval, but parallel activation with:
        - Competition between markers
        - Temporal dynamics (recent markers activated faster)
        - Context gating
        """
        relevant = []
        ...
        for marker in self.markers:
            ...
            if similarity >= threshold:
                # Recency bonus (recent markers retrieve faster/stronger)
                time_since_use = self.current_time - marker.last_activated
                recency_factor = np.exp(-time_since_use / 3600)  # Decay over 1 hour
                
                # Effective strength
                effective_strength = marker.strength * (0.7 + 0.3 * recency_factor)
                
                # Competition: only strong markers survive
        ...
        # Limit to top 5 (working memory constraint)
        return relevant[:5]
    ...
    def _update_autonomic_from_markers(self, markers: List[BiologicalSomaticMarker]):
        """
        Update autonomic state based on activated markers
        
        Markers trigger autonomic responses through:
        - vmPFC → Hypothalamus → ANS
        ...
        # Update vagal tone from HRV (parasympathetic cardiac control)
        self.autonomic.vagal_tone = self.autonomic.parasympathetic_tone * 0.7 + 0.3
    
    def _update_physiology(self, dt: float):
        """
        Update physiological state based on autonomic tone
        
        Models actual physiological changes, not just abstract numbers
        """
        # Heart rate: Sympathetic increases, parasympathetic decreases
        target_hr = (
            60 + 40 * self.autonomic.sympathetic_tone -  # Symp: 60-100 BPM
            20 * self.autonomic.parasympathetic_tone      # Parasymp: decrease
        )
        
        # HR changes with realistic dynamics (seconds time constant)
        tau_hr = 2.0  # 2 second time constant
        self.physiology.heart_rate += (target_hr - self.physiology.heart_rate) * (dt / tau_hr)
        
        # HRV: Higher parasympathetic = higher HRV
        ...
        # Cortisol: Stress hormone (slow changes)
        target_cortisol = 10 + 15 * self.autonomic.sympathetic_tone
        tau_cortisol = 300.0  # 5 minutes
    ...
    def _update_neurotransmitters(self, 
                                 markers: List[BiologicalSomaticMarker],
                                 dt: float):
        """
        Update neurotransmitter levels based on markers and neural activity
        """
        if not markers:
            # Return to baseline
        ...
        # Average marker neurotransmitter signatures
        avg_da_response = np.mean([m.dopamine_response for m in markers])
        avg_5ht_response = np.mean([m.serotonin_response for m in markers])
        avg_ne_response = np.mean([m.norepinephrine_response for m in markers])
        
        # Dopamine: Phasic burst for reward prediction
        # VTA activation drives DA release
        vta_drive = self.neural_circuit.VTA.activation
        target_da_phasic = avg_da_response + vta_drive * 0.5
        tau_da = 0.2  # Fast DA dynamics
        self.neurotransmitters.dopamine_phasic += (target_da_phasic - self.neurotransmitters.dopamine_phasic) * (dt / tau_da)
        
        # Serotonin: Modulated by valence
        avg_valence = np.mean([m.emotional_valence for m in markers]) if markers else 0
        target_5ht = 1.0 + avg_valence * 0.3 + avg_5ht_response * 0.2
        tau_5ht = 10.0  # Slow 5HT changes
        self.neurotransmitters.serotonin_level += (target_5ht - self.neurotransmitters.serotonin_level) * (dt / tau_5ht)
        
        # Norepinephrine: LC activation drives NE release (arousal)
        lc_drive = self.neural_circuit.locus_coeruleus.activation
        target_ne = 1.0 + lc_drive * 0.8 + avg_ne_response * 0.2
        tau_ne = 1.0
    ...
    def _homeostatic_regulation(self, dt: float):
        """
        Homeostatic processes that restore baseline state
        
        Models body's attempt to maintain equilibrium
        """
        ...
        # Blood pressure homeostasis (baroreceptor reflex)
    ...
    def _calculate_current_arousal(self) -> float:
        ...
        Arousal is multi-dimensional:
        - Cardiovascular (HR, HRV)
        - Electrodermal (SCR, SCL)
        - Respiratory
        """
    ...
    def _record_state(self):
        ...
        self.physiological_history.append({
    ...
    def _generate_response(self, markers: List[BiologicalSomaticMarker]) -> Dict[str, Any]:
        """
        Generate complete response with all biological data
        """
        if not markers:
            avg_valence = 0.0
    ...
    def learn_from_outcome(self,
        ...
        """
        Learn from outcome with biological realism
        
        Models:
        - Fast amygdala learning (emotional events)
        - Slow cortical consolidation
        - Reconsolidation (updating existing markers)
        """
        # Find similar marker
        similar_marker = None
        best_similarity = 0.0
        ...
        if similar_marker:
            # Reconsolidation (update existing marker)
            # Fast learning if amygdala active (emotional)
            if abs(outcome_valence) > 0.5:
                alpha = self.learning_rate_fast
            else:
                alpha = self.learning_rate_slow
            ...
            # Update biological signatures (what the body typically does)
            ...
            # Create new marker
            new_marker = BiologicalSomaticMarker(
                ...
                # Capture current physiological signature
                ...
                # Capture neurotransmitter signature
    ...
    def get_biological_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'autonomic': {
                'balance': f"{'Sympathetic' if self.autonomic.autonomic_balance > 0 else 'Parasympathetic'} dominant",
                'sympathetic_tone': f"{self.autonomic.sympathetic_tone:.2f}",
                'parasympathetic_tone': f"{self.autonomic.parasympathetic_tone:.2f}",
                'vagal_tone': f"{self.autonomic.vagal_tone:.2f}"
            },
            'neural': {
