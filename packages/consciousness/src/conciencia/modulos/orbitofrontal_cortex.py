...
class OrbitofrontalCortex:
    ...
    def evaluate_experience(self, experience_data: Dict[str, Any]) -> float:
        """
        Evaluate a whole conscious experience (meta-cognitive evaluation).
        Computes value based on context, arousal, salience, novelty, etc.
        """
        # Simple meta-cognitive value function
        base_value = 0.0

        # Factor 1: Arousal (temporally significant experiences)
        arousal = experience_data.get('arousal', 0.5)
        base_value += arousal * 0.2

        # Factor 2: Salience (attention-grabbing nature)
        salience = experience_data.get('salience', 0.5)
        base_value += salience * 0.3

        # Factor 3: Novelty (learning opportunities)
        novelty = experience_data.get('novelty', 0.3)
        base_value += novelty * 0.4

        # Factor 4: Emotional relevance (personal significance)
        context = experience_data.get('context', {})
        emotional_valence = context.get('emotional_valence', 0.0)
        base_value += abs(emotional_valence) * 0.3

        # Store and retrieve experience value
        exp_id = f"experience_meta_{hash(str(experience_data))}"
        ve = self._get_value(exp_id)
        ve.expected_value = base_value
        ve.confidence = 0.6  # Meta-cognitive inherently uncertain
