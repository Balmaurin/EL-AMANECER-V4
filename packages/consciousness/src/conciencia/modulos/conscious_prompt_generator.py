...
# Integración con Metacognición Lingüística
try:
    from .linguistic_metacognition_system import analyze_and_select_style
    LINGUISTIC_METACOGNITION_AVAILABLE = True
except ImportError:
    LINGUISTIC_METACOGNITION_AVAILABLE = False
...
class ConsciousPromptGenerator:
    ...
    def generate_prompt(self, query: str, context: Optional[Dict[str, Any]] = None,
        ...
        # PASO 0: Análisis Metacognitivo Lingüístico (NUEVO)
        linguistic_analysis = None
        style_selection = None
        
        if LINGUISTIC_METACOGNITION_AVAILABLE:
            try:
                linguistic_analysis, style_selection = analyze_and_select_style(query, context)
                
                # Actualizar contexto con insights lingüísticos
                context['linguistic_intent'] = linguistic_analysis.intent.value
                context['linguistic_complexity'] = linguistic_analysis.linguistic_complexity
                context['emotional_charge'] = linguistic_analysis.emotional_charge
                
                # Si la confianza es baja, sugerir clarificación en las instrucciones
                if linguistic_analysis.confidence < 0.45:
                    clarification_note = " (NOTA: La intención del usuario es ambigua. Si es necesario, pide clarificación amablemente)."
                    if instructions:
                        instructions += clarification_note
                    else:
                        instructions = clarification_note.strip()
                        
                logger.info(f"🧠 Linguistic Analysis: {linguistic_analysis.intent.value} | Style: {style_selection.primary_style.value}")
                
            except Exception as e:
                logger.warning(f"⚠️ Error in linguistic analysis: {e}")
        ...
        # Inyectar instrucciones de estilo si están disponibles
        if style_selection:
            style_instruction = f"\n[ESTILO DE RESPUESTA]: {style_selection.primary_style.value} ({style_selection.emotional_tone})"
            style_instruction += f"\n[RAZÓN]: {style_selection.rationale}"
            
            # Insertar antes de la consulta o al final de las instrucciones
            candidate_prompt = candidate_prompt.replace("[INSTRUCCIONES]:", f"[INSTRUCCIONES]:{style_instruction}")
