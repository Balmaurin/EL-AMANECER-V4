...
class FEPEngine:
    ...
    def process_observation(self, 
        ...
        obs_vector = np.array([observation[k] for k in obs_keys])
    ...
    def _generate_predictions(self, observation: np.ndarray) -> List[np.ndarray]:
        ...
        # Resize states if needed (first observation)
        if len(self.generative_model.states[0].prediction) != len(observation):
            for state in self.generative_model.states:
                state.prediction = np.zeros(len(observation))
                state.prediction_error = np.zeros(len(observation))
