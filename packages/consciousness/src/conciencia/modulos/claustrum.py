...
class ClaustrumExtended:
    ...
    def __init__(
        ...
        self.sync_threshold = float(np.clip(synchronization_threshold, 0.0, 1.0))
        self.arousal: float = 0.5
        self.binding_count: int = 0
        self.failed_binding_count: int = 0
        self.coherence_history: List[float] = []
        self.db = ClaustrumDB(db_path=db_path)
        self.on_unified_cb: Optional[Callable[[Dict[str, Any], Dict[str, Any]], None]] = None
        self.out_queue: Optional[Queue] = None
        self._in_queue: "Queue[Dict[str, Any]]" = Queue()
        self._running: bool = False
        self._rt_thread: Optional[threading.Thread] = None

    def connect_area(self, area_id: str, modality: str, weight: float = 1.0):
        """Register or update a cortical area connected to the claustrum.

        This matches the usage in BiologicalConsciousnessSystem, which
        connects several high-level cortical regions (visual, auditory, etc.)
        to the claustrum for multimodal binding.
        """
        self.areas[area_id] = CorticalArea(area_id=area_id, modality=modality, weight=weight)
        logger.info("Connected area %s (modality=%s, weight=%.2f)", area_id, modality, weight)
