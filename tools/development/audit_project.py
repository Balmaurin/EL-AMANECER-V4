...
class SheilyAuditor:
    ...
    def audit_project_structure(self) -> AuditResult:
        ...
        if missing_files:
            ...
            recommendations.append(
                "Crear archivos críticos faltantes para proyecto profesional"
            )
        ...
        if missing_dirs:
            findings.append(
                f"Directorios esperados faltantes: {', '.join(missing_dirs)}"
            )
        ...
        return self.log_audit(
            "Estructura del Proyecto", status, score, findings, recommendations
        )
    ...
    def audit_dependencies(self) -> AuditResult:
        ...
        try:
            ...
            if missing_critical:
                findings.append(
                    f"Dependencias críticas faltantes: {', '.join(missing_critical)}"
                )

            # Verificar formato de versiones
            unversioned = [
                line for line in lines if not any(char in line for char in ">=<=")
            ]
            if unversioned:
    ...
    def audit_docker_configuration(self) -> AuditResult:
        ...
        issues = len(
            [
                f
                for f in findings
                if "Error" in f or "mal formateado" in f or "no encontrado" in f
            ]
        )
    ...
    def audit_security(self) -> AuditResult:
        ...
        if found_sensitive:
            findings.append(
                f"Archivos sensibles encontrados: {', '.join(found_sensitive)}"
            )
        ...
        try:
            ...
            if result.stdout.strip():
                findings.append(
                    "Algunos archivos tienen permisos de escritura para otros"
                )
        except Exception:
            pass
    ...
    def audit_performance(self) -> AuditResult:
        ...
        for file_path in self.project_root.rglob("*"):
            if (
                file_path.is_file() and file_path.stat().st_size > 50 * 1024 * 1024
            ):  # 50MB
                large_files.append(
        ...
        if modules_without_init:
            findings.append(
                f"Módulos Python sin __init__.py: {len(modules_without_init)}"
            )
    ...
    def audit_documentation(self) -> AuditResult:
        ...
        # Calcular puntuación
        issues = len(missing_docs) + sum(
            1 for f in findings if "corto" in f or "no incluye" in f
        )
    ...
    def audit_code_quality(self) -> AuditResult:
        ...
        init_percentage = (
            (modules_with_init / total_python_files * 100)
        ...
        if large_python_files:
            findings.append(
                f"Archivos Python grandes (>10KB): {len(large_python_files)}"
            )
    ...
    def run_complete_audit(self) -> ProjectAudit:
        ...
        summary = {
            ...
            "recommendations_count": sum(
                len(audit.recommendations) for audit in audits
            ),
        }
    ...
    def generate_report(self, audit: ProjectAudit) -> str:
        ...
        report_lines.append("-" * 40)
        report_lines.append(
            f"• Componentes auditados: {audit.summary['total_components']}"
        )
        report_lines.append(
            f"• Estado PASS: {audit.summary['status_distribution']['PASS']}"
        )
        report_lines.append(
            f"• Estado WARN: {audit.summary['status_distribution']['WARN']}"
        )
        report_lines.append(
            f"• Estado FAIL: {audit.summary['status_distribution']['FAIL']}"
        )
        report_lines.append(
            f"• Recomendaciones totales: {audit.summary['recommendations_count']}"
        )
        report_lines.append(
        ...
        if audit.overall_score >= 90:
            ...
            conclusion = (
                "⚠️ ACEPTABLE - El proyecto necesita mejoras en algunos aspectos"
            )
        else:
            conclusion = (
                "❌ REQUIERE ATENCIÓN - El proyecto necesita revisión significativa"
            )
    ...
    def save_audit_report(self, audit: ProjectAudit, output_file: str = None):
        ...
        txt_path = (
            reports_dir / f"audit_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
        )
