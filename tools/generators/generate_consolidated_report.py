...
def analyze_cross_references(lora_data, corpus_data):
    ...
    corpus_by_branch = {
        item["branch"]: item for item in corpus_details if "branch" in item
    }
    lora_by_branch = {}
...
def generate_recommendations(lora_data, corpus_data, cross_analysis):
    ...
    corpus_details = corpus_data.get("details", [])
    poor_data_branches = [
        item for item in corpus_details if item.get("data_quality") == "poor"
    ]
...
def main():
    ...
    consolidated_report = {
        ...
        "executive_summary": {
            ...
            "corpus_excellent": corpus_data.get("summary", {}).get(
                "excellent_quality", 0
            ),
            ...
            "ready_for_production": len(
                cross_analysis.get("functional_with_good_data", [])
            ),
            "needs_retraining": len(cross_analysis.get("corrupted_with_good_data", [])),
            "needs_data_improvement": len(
                cross_analysis.get("functional_with_poor_data", [])
            ),
        },
    ...
    try:
        with open(
            "audit_2024/reports/consolidated_report.json", "w", encoding="utf-8"
        ) as f:
